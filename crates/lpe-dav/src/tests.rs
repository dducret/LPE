...
async fn get_serializes_organizer_and_participant_status() {
    ...
    let event = AccessibleEvent {
        ...
        ..FakeStore::accessible_event(&FakeStore::calendar_collection(), event_id, "Status review")
    };
    let store = FakeStore {
        session: Some(FakeStore::account()),
        events: Arc::new(Mutex::new(vec![event])),
    ...
    let response = service
        .handle(
            &Method::GET,
            &Uri::from_static("/dav/calendars/me/default/abababab-abab-abab-abab-abababababab.ics"),
            &bearer_headers(),
            &[],
        )
        .await
