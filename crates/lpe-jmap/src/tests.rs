...
use lpe_storage::{
    ...
    CanonicalChangeReplay, CanonicalPushChangeSet, ClientContact, ClientEvent, ClientTaskList,
    CollaborationCollection, CollaborationRights, CreateTaskListInput, JmapEmailAddress,
    JmapEmailMailboxState, JmapEmailSubmission, JmapImportedEmailInput, JmapMailObjectChange,
    JmapMailboxCreateInput, JmapMailboxUpdateInput, JmapQuota, JmapStoredQueryState,
    JmapStringObjectChange, MailboxAccountAccess, MailboxRule, OutlookProfileState,
    RecipientSuggestion, SavedDraftMessage, SearchFolderDefinition, SenderIdentity,
...
impl FakeStore {
    ...
    fn draft_email() -> JmapEmail {
        ...
        JmapEmail {
            ...
            mime_blob_ref: Some("draft-message:cccccccc-cccc-cccc-cccc-cccccccccccc".to_string()),
...
impl JmapStore for FakeStore {
    ...
    async fn copy_jmap_email_between_accounts(
        ...
        email.mailbox_states = vec![JmapEmailMailboxState {
            ...
            reminder_set: email.reminder_set,
            reminder_at: email.reminder_at.clone(),
...
async fn session_and_identity_include_accessible_shared_mailbox_accounts() {
    ...
    let shared_capabilities =
        &session.accounts[&FakeStore::shared_account().account_id.to_string()].account_capabilities;
...
async fn mailbox_and_email_changes_return_existing_ids_from_initial_state() {
    ...
    let service =
        JmapService::new_with_validator(store, validator_ok("message/rfc822", "eml", "eml", 0.99));
...
async fn thread_changes_use_durable_log_ids_when_state_has_cursor() {
    ...
    let prior_state = encode_state_with_cursor(
        ...
        "Thread",
        vec![StateEntry {
            id: FakeStore::draft_email().thread_id.to_string(),
            fingerprint: "old-thread".to_string(),
        }],
        Some(9),
    )
    .unwrap();
    let store = FakeStore {
        ...
        jmap_mail_object_changes: Arc::new(Mutex::new(Some(vec![JmapMailObjectChange {
            cursor: 10,
            object_id: FakeStore::draft_email().thread_id,
            change_kind: "updated".to_string(),
        }]))),
        ..Default::default()
    };
    ...
    assert_eq!(
        response.method_responses[0].1["updated"],
        json!([FakeStore::draft_email().thread_id.to_string()])
    );
}
...
async fn email_submission_changes_use_durable_log_ids_when_state_has_cursor() {
    ...
    let service = JmapService::new(FakeStore {
        session: Some(FakeStore::account()),
        email_submissions: vec![queued.clone()],
        canonical_change_cursor: Some(32),
    ...
    let decoded =
        decode_state(response.method_responses[0].1["newState"].as_str().unwrap()).unwrap();
    assert_eq!(decoded.cursor, Some(32));
}
...
async fn mailbox_copy_and_import_reject_read_only_shared_mailbox_mutations() {
    ...
    assert_eq!(
        response.method_responses[0].1["notDestroyed"][FakeStore::draft_mailbox().id.to_string()]
            ["description"],
        "write access is not granted on this mailbox account"
    );
    assert_eq!(
...
async fn blob_get_hides_bcc_for_delegated_shared_mailbox_message() {
    ...
    let digest =
        base64::engine::general_purpose::STANDARD.encode(sha2::Sha256::digest(message.as_bytes()));
    assert_eq!(
...
async fn api_request_rejects_batches_beyond_advertised_call_limit() {
    ...
    let method_calls = (0..=MAX_CALLS_IN_REQUEST)
        .map(|index| JmapMethodCall("Mailbox/query".to_string(), json!({}), format!("c{index}")))
        .collect();
...
async fn websocket_push_states_include_submission_identity_mail_types() {
    ...
    assert!(states[&FakeStore::account().account_id.to_string()].contains_key("EmailSubmission"));
        ...
        .handle_api_request(
            ...
            JmapApiRequest {
                ...
                method_calls: vec![
                    ...
                    JmapMethodCall("AddressBook/query".to_string(), json!({}), "c3".to_string()),
                    JmapMethodCall("ContactCard/query".to_string(), json!({}), "c4".to_string()),
                    JmapMethodCall(
                        "ContactCard/set".to_string(),
                        json!({
                            "create": {
...
#[tokio::test]
async fn collaboration_changes_use_durable_log_ids_when_state_has_cursor() {
    let account_id = FakeStore::account().account_id;
    let contact_state = encode_state_with_cursor(
        account_id,
        "ContactCard",
        vec![StateEntry {
            id: FakeStore::contact().id.to_string(),
            fingerprint: "old-contact".to_string(),
        }],
        Some(31),
    )
    .unwrap();
    let event_state = encode_state_with_cursor(
        account_id,
        "CalendarEvent",
        vec![StateEntry {
            id: FakeStore::event().id.to_string(),
            fingerprint: "old-event".to_string(),
        }],
        Some(31),
    )
    .unwrap();
    let store = FakeStore {
        ...
        events: Arc::new(Mutex::new(vec![FakeStore::event()])),
        canonical_change_cursor: Some(32),
        jmap_object_changes: Arc::new(Mutex::new(Some(vec![
            JmapMailObjectChange {
                cursor: 32,
                object_id: FakeStore::contact().id,
                change_kind: "updated".to_string(),
            },
            JmapMailObjectChange {
                cursor: 32,
                object_id: FakeStore::event().id,
                change_kind: "updated".to_string(),
            },
        ]))),
        ..Default::default()
        ...
        .handle_api_request(
            ...
            JmapApiRequest {
                ...
                method_calls: vec![
                    JmapMethodCall(
                        "ContactCard/changes".to_string(),
                        json!({"sinceState": contact_state}),
                        "cc".to_string(),
                    ),
                    JmapMethodCall(
                        "CalendarEvent/changes".to_string(),
                        json!({"sinceState": event_state}),
                        "ce".to_string(),
                    ),
                ],
            },
    ...
    assert_eq!(
        response.method_responses[0].1["updated"],
        json!([FakeStore::contact().id.to_string()])
    );
    assert_eq!(
        response.method_responses[1].1["updated"],
        json!([FakeStore::event().id.to_string()])
    );
}
...
async fn upload_rejects_declared_mime_mismatch() {
    ...
    let service =
        JmapService::new_with_validator(store, validator_ok("application/pdf", "pdf", "pdf", 0.99));
...
async fn upload_surfaces_magika_failure_mode() {
    ...
    let service = JmapService::new_with_validator(store, validator_error("Magika command failed"));
