    ...
    V: Detector + Clone + Send + Sync + 'static,
{
    pub(in crate::service) async fn upload_items(
        ...
        for upload in uploads {
            ...
            if validation.policy_decision != PolicyDecision::Accept {
                bail!(
                    "UploadItems blocked by Magika validation: {}",
                    validation.reason
                );
            }
            ...
            for attachment in &parsed.attachments {
                ...
                if validation.policy_decision != PolicyDecision::Accept {
                    bail!(
                        "UploadItems attachment '{}' blocked by Magika validation: {}",
    ...
    pub(in crate::service) async fn export_items(
        ...
        if item_ids.is_empty() || item_ids.len() > MAX_TRANSFER_ITEMS {
            bail!(
                "ExportItems requires between one and {MAX_TRANSFER_ITEMS} canonical ItemId values"
            );
        }
        let message_ids = item_ids
            .iter()
            .map(|item_id| parse_transfer_message_id(item_id))
            .collect::<Result<Vec<_>>>()?;
        let exports = self
            .store
            .fetch_ews_transfer_exports(principal, &message_ids)
            .await?;
...
fn parse_transfer_mailbox_id(value: &str) -> Result<Uuid> {
    value
        ...
        .parse()
        .context("UploadItems ParentFolderId is not a canonical mailbox id")
}
...
fn parse_transfer_message_id(value: &str) -> Result<Uuid> {
    value
        ...
        .unwrap_or(value.trim())
        .parse()
        .context("ExportItems ItemId is not a canonical message id")
}
...
fn strip_bcc_headers(raw_message: &[u8]) -> Vec<u8> {
    ...
    for line in source.lines() {
        ...
        if line.to_ascii_lowercase().starts_with("bcc:") {
            skipping = true;
            continue;
        }
