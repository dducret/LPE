    ...
    () => {
        ...
        fn upsert_mapi_special_folder_aliases<'a>(
            ...
            Box::pin(async move {
                ...
                let change_numbers = upsert_mapi_special_folder_aliases_in_tx(
                    &mut tx,
                    tenant_id,
                    account_id,
                    store_identity,
...
async fn upsert_mapi_special_folder_aliases_in_tx(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    tenant_id: Uuid,
    account_id: Uuid,
    store_identity: lpe_storage::MapiStoreIdentity,
    aliases: &[MapiSpecialFolderAlias],
) -> Result<Vec<u64>> {
    ...
    for alias in aliases {
        ...
        if alias.alias_folder_id == alias.canonical_folder_id
            ...
            || alias.source_key.get(..16) != Some(store_identity.replica_guid.as_bytes().as_slice())
        ...
        if let Some(existing) = existing_alias {
            if existing.get::<i64, _>("alias_folder_id") as u64 == alias.alias_folder_id
                && existing.get::<i64, _>("canonical_folder_id") as u64 == alias.canonical_folder_id
                && existing.get::<Vec<u8>, _>("source_key") == alias.source_key
            {
                change_numbers.push(existing.get::<i64, _>("mapi_change_number") as u64);
                continue;
            }
