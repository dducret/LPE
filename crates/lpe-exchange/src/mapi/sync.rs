...
use delegate_freebusy::delegate_freebusy_sync_object;
        ...
        .is_some_and(|folder| folder.kind == crate::mapi_store::MapiCollaborationFolderKind::Task)
    {
        ...
        objects.extend(match folder_id {
            ...
            FREEBUSY_DATA_FOLDER_ID => snapshot
                ...
                .iter()
                .map(|message| delegate_freebusy_sync_object(message, principal.account_id))
                .collect(),
            _ => Vec::new(),
        });
    }
...
pub(in crate::mapi) fn associated_sync_message_exists(
    ...
    match folder_id {
        ...
        FREEBUSY_DATA_FOLDER_ID => snapshot
            ...
            .is_some_and(delegate_freebusy_message_is_associated),
        _ => false,
    }
}
    ...
    match object {
        ...
        MapiObject::DelegateFreeBusyMessage {
            ...
            if let Some(state) = saved_state {
                effective_message.durable_identity = Some(state.identity.clone());
                effective_message.custom_properties = state.custom_properties.clone();
            }
            let message = &effective_message;
            let local_freebusy = crate::mapi_store::is_outlook_local_freebusy_message(message);
            let entry_id = (!local_freebusy)
                .then(|| {
                    crate::mapi::identity::message_entry_id_from_object_ids(
                        principal.account_id,
                        message.folder_id,
                ...
                .flatten();
            let parent_entry_id = (!local_freebusy)
                .then(|| {
                    crate::mapi::identity::folder_entry_id_from_object_id(
                        principal.account_id,
                        message.folder_id,
            ...
            Some((
                *folder_id,
                mapi_mailstore::fast_transfer_message_content_buffer_with_special_object(
                    ...
                    &special_message_with_named_property_definitions(
                        delegate_freebusy_sync_object(&message, principal.account_id),
                        snapshot,
                    ),
                    send_options,
                    direct_property_filter,
