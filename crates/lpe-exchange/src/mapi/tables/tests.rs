...
fn inbox_associated_invariant_uses_mailbox_guid_entry_id() {
    ...
    let summaries = outlook_bootstrap_row_invariant_summaries(
        ...
        &[],
        &[],
        &inbox_associated_sort_snapshot(),
        mailbox_guid,
        true,
        1,
    );
...
#[test]
fn empty_common_views_has_no_row_identity_invariant() {
    let mailbox_guid = Uuid::parse_str("bc737006-4413-49b9-aefc-3cb6e0088492").unwrap();
    let object = MapiObject::ContentsTable {
...
#[test]
fn common_views_find_row_does_not_invent_default_compact_named_view() {
    let account_id = Uuid::from_u128(0xea33944627b94a9cb0de873f03a35376);
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: COMMON_VIEWS_FOLDER_ID,
...
#[test]
fn common_views_find_row_does_not_invent_default_sent_to_named_view() {
    let account_id = Uuid::from_u128(0xea33944627b94a9cb0de873f03a35376);
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: COMMON_VIEWS_FOLDER_ID,
...
#[test]
fn common_views_wlink_query_rows_do_not_add_named_views_without_restriction() {
    let account_id = Uuid::from_u128(0xea33944627b94a9cb0de873f03a35376);
    let snapshot = common_views_sort_snapshot(account_id);
    let mut table = MapiObject::ContentsTable {
        folder_id: COMMON_VIEWS_FOLDER_ID,
...
#[test]
fn common_views_restricted_named_view_query_rows_are_empty_without_persisted_fai() {
    let account_id = Uuid::from_u128(0xea33944627b94a9cb0de873f03a35376);
    let snapshot = common_views_sort_snapshot(account_id);
    let mut table = MapiObject::ContentsTable {
        folder_id: COMMON_VIEWS_FOLDER_ID,
...
fn captured_common_views_query_rows_flags_heterogeneous_missing_columns() {
    ...
    assert!(display_name_position("Calendar") < display_name_position("Contacts"));
...
fn common_views_query_rows_projects_mailbox_store_object_entry_id() {
    ...
    let shortcut_id = Uuid::from_u128(0x6d617069_776c_496e_8000_000000000099);
    let shortcut_object_id = crate::mapi::identity::mapi_store_id(
        crate::mapi::identity::FIRST_DYNAMIC_GLOBAL_COUNTER + 99,
    );
    crate::mapi::identity::remember_mapi_identity(shortcut_id, shortcut_object_id);
    let snapshot = MapiMailStoreSnapshot::empty().with_navigation_shortcuts(vec![
        crate::store::MapiNavigationShortcutRecord {
            id: shortcut_id,
            account_id,
            ...
            group_name: "Favorites".to_string(),
            client_properties: crate::store::MapiNavigationShortcutClientProperties::default(),
        },
    ]);
    let mut table = MapiObject::ContentsTable {
...
#[test]
fn inbox_associated_find_row_returns_not_found_for_unpersisted_named_view() {
...
#[test]
fn inbox_associated_find_row_returns_not_found_for_unstored_sharing_configuration() {
    assert_inbox_associated_find_row_no_match_for_message_class("IPM.Sharing.Configuration");
}

#[test]
fn inbox_associated_exact_virtual_find_row_does_not_inject_a_row() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: INBOX_FOLDER_ID,
        associated: true,
...
#[test]
fn inbox_associated_find_row_returns_not_found_for_unstored_sharing_index() {
    assert_inbox_associated_find_row_no_match_for_message_class("IPM.Sharing.Index");
}

#[test]
fn inbox_associated_find_row_returns_not_found_for_unstored_aggregation_config() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: INBOX_FOLDER_ID,
        associated: true,
...
#[test]
fn empty_snapshot_does_not_contain_virtual_sharing_index() {
    let snapshot = MapiMailStoreSnapshot::empty();
    assert!(snapshot
...
#[test]
fn inbox_associated_find_row_does_not_invent_a_default_for_a_broad_startup_lookup() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: INBOX_FOLDER_ID,
        associated: true,
...
#[test]
fn contacts_associated_find_row_does_not_invent_osc_contact_sync_config() {
...
#[test]
fn contacts_associated_find_row_does_not_invent_contact_link_timestamp_config() {
    assert_contact_folder_associated_find_row_does_not_return_config(
        CONTACTS_FOLDER_ID,
...
fn contacts_associated_find_row_preserves_table_position_for_contact_link_timestamp() {
    ...
    let contact_link_id = Uuid::from_u128(0x6d617069_636c_696e_8000_000000000001);
    crate::mapi::identity::remember_mapi_identity(
        contact_prefs_id,
        crate::mapi::identity::mapi_store_id(
...
#[test]
fn inbox_associated_table_omits_unpersisted_default_named_view_for_exact_lookup() {
    let restriction = MapiRestriction::Property {
        relop: 0x04,
        property_tag: PID_TAG_MESSAGE_CLASS_W,
...
#[test]
fn inbox_associated_table_omits_unpersisted_default_named_view_without_restriction() {
    let rows = associated_table_rows(
        INBOX_FOLDER_ID,
        &MapiMailStoreSnapshot::empty(),
        None,
...
#[test]
fn quick_contacts_associated_find_row_does_not_invent_osc_contact_sync_config() {
    assert_contact_folder_associated_find_row_does_not_return_config(
        QUICK_CONTACTS_FOLDER_ID,
        "IPM.Microsoft.OSC.ContactSync",
        &MapiMailStoreSnapshot::empty(),
...
#[test]
fn im_contact_list_associated_find_row_does_not_invent_osc_contact_sync_config() {
    assert_contact_folder_associated_find_row_does_not_return_config(
        IM_CONTACT_LIST_FOLDER_ID,
        "IPM.Microsoft.OSC.ContactSync",
        &MapiMailStoreSnapshot::empty(),
...
fn dynamic_contacts_associated_find_row_does_not_invent_osc_contact_sync_config() {
    ...
    assert_eq!(u32::from_le_bytes(row.try_into().unwrap()), 0);
}

#[test]
fn mailbox_backed_quick_contacts_associated_find_row_does_not_invent_osc_contact_sync_config() {
...
#[test]
fn inbox_associated_exact_named_view_find_row_respects_existing_table_restriction() {
    let snapshot = MapiMailStoreSnapshot::empty();
...
fn inbox_associated_find_row_followup_uses_the_original_rowset() {
    ...
    assert_response_contains_utf16(&find_response, "IPM.Configuration.ClientOptions");
    assert_eq!(table_position(&table), Some(0));
...
#[test]
fn inbox_associated_greater_than_restriction_uses_normal_property_semantics() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let restriction = MapiRestriction::Property {
        relop: 0x02,
        property_tag: PID_TAG_MESSAGE_CLASS_W,
...
#[test]
fn inbox_associated_content_restriction_projects_persisted_configs() {
    let account_id = Uuid::from_u128(0xea33944627b94a9cb0de873f03a35376);
    ...
    let mut classes = rows
        .iter()
        .map(associated_table_row_config)
    ...
    let unrestricted_rows = associated_table_rows(INBOX_FOLDER_ID, &snapshot, None, Uuid::nil());
    let mut unrestricted_classes = unrestricted_rows
        .iter()
        .map(associated_table_row_config)
        .map(|message| message.message_class.as_str())
        .collect::<Vec<_>>();
    unrestricted_classes.sort_unstable();
    ...
    let exact_autocomplete = MapiRestriction::Property {
        relop: 0x04,
        property_tag: PID_TAG_MESSAGE_CLASS_W,
        value: MapiValue::String("IPM.Configuration.Autocomplete".to_string()),
    };
    let exact_rows = associated_table_rows(
        INBOX_FOLDER_ID,
        &snapshot,
        Some(&exact_autocomplete),
        Uuid::nil(),
    );
    let exact_classes = exact_rows
        .iter()
        .map(associated_table_row_config)
        .map(|message| message.message_class.as_str())
        .collect::<Vec<_>>();

    assert_eq!(exact_classes, vec!["IPM.Configuration.Autocomplete"]);
}

#[test]
fn inbox_associated_restriction_does_not_add_a_modeled_startup_class() {
    let snapshot = inbox_associated_sort_snapshot();
    let restriction = MapiRestriction::Property {
        relop: 0x02,
        property_tag: PID_TAG_MESSAGE_CLASS_W,
    ...
    assert_eq!(message_list_settings_count, 0);
}
...
fn inbox_associated_find_row_can_return_a_persisted_extended_rule_message() {
    ...
    assert_eq!(response[0], RopId::FindRow.as_u8());
    assert_eq!(u32::from_le_bytes(response[2..6].try_into().unwrap()), 0);
    assert_response_contains_utf16(&response, "IPM.ExtendedRule.Message");
}
...
#[test]
fn inbox_associated_query_rows_includes_persisted_extended_rule_message() {
    let snapshot = inbox_associated_extended_rule_snapshot();
    let mut table = MapiObject::ContentsTable {
        folder_id: INBOX_FOLDER_ID,
        associated: true,
...
#[test]
fn junk_associated_query_rows_do_not_invent_default_named_view() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
...
#[test]
fn contacts_associated_query_rows_do_not_invent_default_named_view_or_helpers() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: CONTACTS_FOLDER_ID,
        associated: true,
...
fn calendar_associated_query_rows_prefix_configuration_returns_calendar_config() {
    ...
    assert!(utf16_position(&response, "IPM.Configuration.AvailabilityOptions").is_some());
}

#[test]
fn inbox_associated_query_rows_prefix_configuration_exposes_message_list_settings() {
    let account_id = Uuid::from_u128(0xea33944627b94a9cb0de873f03a35376);
...
#[test]
fn inbox_associated_query_rows_does_not_create_virtual_umolk_user_options() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: INBOX_FOLDER_ID,
        associated: true,
...
#[test]
fn virtual_umolk_user_options_reports_optional_properties_absent() {
    let message =
    ...
    for tag in [
        ...
        assert_eq!(associated_config_property_value(&message, tag), None);
    }
}

#[test]
fn inbox_associated_query_rows_does_not_create_virtual_mrm_configuration() {
    let snapshot = MapiMailStoreSnapshot::empty();
    let mut table = MapiObject::ContentsTable {
        folder_id: INBOX_FOLDER_ID,
        associated: true,
    ...
    assert_eq!(response[0], RopId::QueryRows.as_u8());
    assert_eq!(u16::from_le_bytes([response[7], response[8]]), 0);
    assert!(utf16_position(&response, "IPM.Configuration.MRM").is_none());
}
...
fn inbox_associated_rows_project_folder_id_and_last_modification_time() {
    ...
    assert!(matches!(
        associated_config_property_value(&stale_dictionary, PID_TAG_ROAMING_DICTIONARY),
        Some(MapiValue::Binary(value))
            if value.starts_with(br#"<?xml version="1.0" encoding="utf-8"?>"#)
                && value.windows(b"18-OLPrefsVersion".len()).any(|window| window == b"18-OLPrefsVersion")
                && value.windows(b"9-1".len()).any(|window| window == b"9-1")
...
#[test]
fn empty_contact_link_timestamp_config_does_not_invent_osc_properties() {
    let message = MapiAssociatedConfigMessage {
...
#[test]
fn persisted_contacts_helper_configs_project_only_stored_helper_properties() {
    for message_class in [
        ...
        assert_eq!(
            ...
            Some(MapiValue::String("2026-07-19T14:00:00Z".to_string()))
        );
        assert_eq!(
            associated_config_property_value(&message, PID_TAG_MESSAGE_CLASS_W),
...
fn common_views_sort_snapshot(account_id: Uuid) -> MapiMailStoreSnapshot {
    ...
    MapiMailStoreSnapshot::empty().with_navigation_shortcuts(vec![
        crate::store::MapiNavigationShortcutRecord {
            ...
            save_stamp: 0,
            section: 1,
            ordinal: crate::mapi::properties::wlink_ordinal_bytes(0x20),
            group_header_id,
            group_name: "Mail".to_string(),
            client_properties: crate::store::MapiNavigationShortcutClientProperties::default(),
        },
...
fn special_folder_property_omits_unpersisted_default_views() {
    ...
    for folder_id in [
        ...
        assert_eq!(
            ...
            None,
            "folder 0x{folder_id:016x} must not point to a synthetic view"
        );
    }
    for folder_id in [
        DEFERRED_ACTION_FOLDER_ID,
