...
fn journal_getprops_flags_absent_web_view_properties() {
    ...
    assert_eq!(
        response,
        vec![
            0x07, 0x02, 0x80, 0x03, 0x04, 0x00, 0x01, 0x0A, 0x0F, 0x01, 0x04, 0x80, 0x0A, 0x0F,
            0x01, 0x04, 0x80,
        ]
    );
}
...
fn inbox_getprops_flags_binary_acl_member_name_as_absent() {
    ...
    assert_eq!(
        response,
        vec![0x07, 0x02, 0x80, 0x03, 0x04, 0x00, 0x01, 0x0A, 0x0F, 0x01, 0x04, 0x80,]
    );
}
...
#[test]
fn getprops_contract_response_summary_reads_errors_returned_row() {
    let summary = getprops_contract_response_summary(
        &[PID_TAG_ACCESS, 0x0E0B_0102],
        &[
            0x07, 0x00, 0x80, 0x03, 0x04, 0x00, 0x01, 0x00, 0x3f, 0x00, 0x00, 0x00, 0x0A, 0x0F,
            0x01, 0x04, 0x80,
        ],
    );

    assert_eq!(summary.result, "0x00040380");
    assert_eq!(summary.returned_tags, "0x0ff40003");
    assert!(summary.problem_tags.contains("0x0e0b0102"));
    assert!(summary.problem_tags.contains("0x8004010f"));
}
