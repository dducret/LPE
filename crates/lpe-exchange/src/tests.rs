...
async fn mapi_over_http_outlook_hierarchy_sync_manifest_includes_folders() {
    ...
    let ipm_parent_source_key =
        mapi_mailstore::source_key_for_store_id(crate::mapi::identity::IPM_SUBTREE_FOLDER_ID);
    let mut root_child_parent_source_key = 0x65E1_0102u32.to_le_bytes().to_vec();
    root_child_parent_source_key
        .extend_from_slice(&(ipm_parent_source_key.len() as u32).to_le_bytes());
    root_child_parent_source_key.extend_from_slice(&ipm_parent_source_key);
    assert!(contains_bytes(
        &response_rops,
        &root_child_parent_source_key
    ));
...
async fn mapi_over_http_folder_set_properties_survive_new_session() {
    ...
    set_rops.extend_from_slice(&folder_id.to_le_bytes());
    set_rops.push(0);
    ...
    get_rops.extend_from_slice(&folder_id.to_le_bytes());
    get_rops.push(0);
...
async fn mapi_over_http_root_default_folder_set_properties_survive_new_session() {
    ...
    set_rops.extend_from_slice(&crate::mapi::identity::ROOT_FOLDER_ID.to_le_bytes());
    set_rops.push(0);
    set_rops.extend_from_slice(&[
    ...
    get_rops.extend_from_slice(&crate::mapi::identity::ROOT_FOLDER_ID.to_le_bytes());
    get_rops.push(0);
    get_rops.extend_from_slice(&[
