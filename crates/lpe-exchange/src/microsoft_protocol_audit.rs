...
const REPORT_ROWS: &[ReportRow] = &[
    ...
    ReportRow {
        ...
        implemented: "Implemented scalar and multivalue property types are manifest-checked. `RopGetPropertiesSpecific` projects `PtypUnspecified` requests as typed values for modeled object properties, including the MS-OXCDATA PropertyRow example.",
        gaps: "`PtypUnspecified` requests for properties not modeled by LPE still return normal property errors. `PtypNull`, floating/currency/object/serverId/restriction/ruleAction and related multivalue forms are explicitly `KnownUnsupported`.",
    },
    ReportRow {
