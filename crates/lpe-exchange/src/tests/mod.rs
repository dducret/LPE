...
async fn postgres_mapi_calendar_property_bag_survives_web_update_and_reload() -> anyhow::Result<()>
{
    ...
    let imported_last_modification_time = 134_309_167_800_000_000;
    ...
    assert_eq!(created.version.change_key, imported_change_key);
    assert_eq!(
    ...
    let imported_times = sqlx::query(
        r#"
        SELECT
            (EXTRACT(EPOCH FROM (
                identity.updated_at - TIMESTAMPTZ '1601-01-01 00:00:00+00'
            )) * 10000000)::bigint AS identity_lmt,
            (EXTRACT(EPOCH FROM (
                event.updated_at - TIMESTAMPTZ '1601-01-01 00:00:00+00'
            )) * 10000000)::bigint AS local_commit_time
    ...
    .await?;
    assert_eq!(
        imported_times.get::<i64, _>("identity_lmt") as u64,
        imported_last_modification_time
    );
    assert_ne!(
        imported_times.get::<i64, _>("local_commit_time") as u64,
        imported_last_modification_time,
        "the imported version LMT must not replace the local commit time"
    );
    ...
    assert_eq!(
        reloaded_event.version.change_key,
        mapi_mailstore::change_key_for_change_number(reloaded_event.version.change_number)
    );
    assert_ne!(reloaded_event.version.change_key, imported_change_key);
    assert_ne!(
        reloaded_event.version.last_modification_time, imported_last_modification_time,
        "a later web modification must rotate the durable identity LMT"
    );
    assert!(
        mapi_mailstore::filetime_from_rfc3339_utc(&reloaded_event.version.updated_at)
            > imported_last_modification_time,
        "a later web modification must advance the independent local commit time"
    );
    assert!(test_mapi_pcl_includes_change_key(
        &reloaded_event.version.predecessor_change_list,
        &imported_change_key
    ));
    assert!(test_mapi_pcl_includes_change_key(
        &reloaded_event.version.predecessor_change_list,
...
async fn mapi_default_calendar_folder_identity_is_persisted() {
    ...
    let calendar_identity = store
        .mapi_identities
        .lock()
        .unwrap()
        .get(&calendar_folder.id)
        .copied()
        .expect("Calendar virtual folder identity");
    assert!(
        crate::mapi::identity::global_counter_from_store_id(calendar_identity)
            .is_some_and(|counter| counter >= crate::mapi::identity::FIRST_DYNAMIC_GLOBAL_COUNTER)
    );
    assert_ne!(calendar_identity, crate::mapi::identity::CALENDAR_FOLDER_ID);
    assert_eq!(
        store
            ...
            .unwrap()
            .get(&calendar_folder.id),
        Some(&crate::mapi::identity::source_key_for_object_id(
            calendar_identity
        ))
    );
...
async fn mapi_full_snapshot_persists_virtual_special_folder_version_identity() {
    ...
    let virtual_identity = store.mapi_identities.lock().unwrap()[&virtual_mailbox.id];
    assert!(
        crate::mapi::identity::global_counter_from_store_id(virtual_identity)
            .is_some_and(|counter| counter >= crate::mapi::identity::FIRST_DYNAMIC_GLOBAL_COUNTER)
    );
    assert_ne!(
        virtual_identity,
...
async fn mapi_identity_repair_removes_orphaned_checkpoint_and_config_state() {
    ...
    let identity = storage
        ...
        .unwrap()
        .remove(0);
    let virtual_special_mailbox_id = crate::mapi_mailstore::virtual_special_mailbox_id(
        crate::mapi::identity::COMMON_VIEWS_FOLDER_ID,
    );
    let virtual_special_identity = storage
        .fetch_or_allocate_mapi_identities(
            account_id,
            &[MapiIdentityRequest {
                object_kind: MapiIdentityObjectKind::Mailbox,
                canonical_id: virtual_special_mailbox_id,
    ...
    let virtual_special_object_id_after_repair = sqlx::query_scalar::<_, i64>(
        r#"
    ...
    .bind(tenant_id)
    .bind(account_id)
    .bind(virtual_special_mailbox_id)
...
impl ExchangeStore for FakeStore {
    ...
    fn fetch_or_allocate_mapi_identities<'a>(
        ...
        Box::pin(async move {
            ...
            for request in requests {
                let reserved_default_folder_counter =
                    request.reserved_global_counter.is_some_and(|counter| {
                        (1..crate::mapi::identity::FIRST_DYNAMIC_GLOBAL_COUNTER).contains(&counter)
                    });
                let (object_id, change_number, allocated_source_key) =
                    if let Some(existing) = identities.get(&request.canonical_id) {
                        ...
                        (*existing, change_number, None)
                    } else {
                        let counter = match request.reserved_global_counter {
                            None => {
                            ...
                            Some(_) if reserved_default_folder_counter => {
                                let value = *next_counter;
                                *next_counter = next_counter.saturating_add(1);
                                value
                            }
                        ...
                        let requested_source_key = if reserved_default_folder_counter {
                            crate::mapi::identity::source_key_for_object_id(object_id)
                        } else {
                            request.source_key.clone().unwrap_or_else(|| {
                                crate::mapi::identity::source_key_for_object_id(object_id)
                            })
                        };
                        ...
                        if reserved_default_folder_counter {
                            source_keys.insert(request.canonical_id, requested_source_key.clone());
                        } else if let Some(source_key) = request.source_key.clone() {
                            source_keys.insert(request.canonical_id, source_key);
                        }
                        let change_number = if request.reserved_global_counter.is_some()
                            && !reserved_default_folder_counter
                        {
                        ...
                        (object_id, change_number, Some(requested_source_key))
                    };
                let source_key = allocated_source_key.unwrap_or_else(|| {
                    if reserved_default_folder_counter {
                        source_keys
                            .get(&request.canonical_id)
                            .cloned()
        ...
        let versions = event_ids
            ...
            .filter_map(|event_id| {
                ...
                let version = MapiEventVersion {
                    ...
                    last_modification_time: test_filetime("2026-07-15", "10:00") as u64,
    ...
    fn create_mapi_event<'a>(
        ...
        *next_counter = next_counter.saturating_add(1);
        ...
        let (change_key, predecessor_change_list, last_modification_time) = imported_identity
            .as_ref()
            .map(|identity| {
                (
            ...
            .unwrap_or_else(|| {
                (
                    ...
                    test_filetime("2026-07-15", "10:00") as u64,
                )
            });
        let version = MapiEventVersion {
            event_id,
        ...
        let (change_key, predecessor_change_list, last_modification_time) = imported_identity
            ...
            .unwrap_or_else(|| {
                (
                    mapi_mailstore::change_key_for_change_number(change_number),
                    mapi_mailstore::predecessor_change_list(change_number),
                    test_filetime("2026-07-15", "10:15") as u64,
                )
            });
        let version = MapiEventVersion {
            event_id: input.event_id,
...
const PID_TAG_INST_ID: u32 = 0x674D_0014;
const PID_TAG_INSTANCE_NUM: u32 = 0x674E_0003;
const PID_TAG_CHANGE_NUMBER: u32 = 0x67A4_0014;
const PID_TAG_DEFAULT_POST_MESSAGE_CLASS_W: u32 = 0x36E5_001F;
const OUTLOOK_IPM_HIERARCHY_FOLDER_COUNT: u32 = 18;
