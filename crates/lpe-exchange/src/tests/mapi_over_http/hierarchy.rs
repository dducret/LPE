...
#[tokio::test]
async fn mapi_over_http_microsoft_oxcfold_create_delete_and_move_use_canonical_mailboxes() {
...
async fn mapi_over_http_delete_staged_search_folder_succeeds_without_persistence() {
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&delete_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let response_rops = response_rops_from_execute_response(response).await;
...
async fn mapi_over_http_copy_folder_is_not_supported_without_side_effects() {
    ...
    let service = ExchangeService::new(store);
    let (mut execute_headers, logon_handle) = mapi_connect_with_private_logon(&service).await;
    for want_recursive in [0x00, 0x01] {
        let mut rops = Vec::new();
        append_rop_open_folder(
            &mut rops,
            0,
...
async fn mapi_over_http_move_folder_rejects_wrong_source_parent_without_side_effects() {
    let source_id = Uuid::parse_str("35353535-3535-4535-8535-353535353536").unwrap();
    let wrong_parent_id = Uuid::parse_str("36363636-3636-4636-8636-363636363637").unwrap();
    let source_mapi_id = test_mapi_uuid_id(&source_id);
    let wrong_parent_mapi_id = test_mapi_uuid_id(&wrong_parent_id);
    crate::mapi::identity::remember_mapi_identity(source_id, source_mapi_id);
    crate::mapi::identity::remember_mapi_identity(wrong_parent_id, wrong_parent_mapi_id);
    let updated_mailboxes = Arc::new(Mutex::new(Vec::new()));
    let store = FakeStore {
        session: Some(FakeStore::account()),
        mailboxes: Arc::new(Mutex::new(vec![
            FakeStore::mailbox(&source_id.to_string(), "custom", "Projects"),
            FakeStore::mailbox(&wrong_parent_id.to_string(), "custom", "Other"),
        ])),
        mapi_identities: Arc::new(Mutex::new(HashMap::from([
            (source_id, source_mapi_id),
            (wrong_parent_id, wrong_parent_mapi_id),
        ]))),
        updated_mailboxes: updated_mailboxes.clone(),
        ..Default::default()
    };
    ...
    let response_rops = response_rops_from_execute_response(response).await;
    assert!(
        contains_bytes(&response_rops, &[0x35, 0x01, 0x0F, 0x01, 0x04, 0x80]),
        "unexpected MoveFolder response: {response_rops:02x?}"
    );
    assert!(updated_mailboxes.lock().unwrap().is_empty());
}

#[tokio::test]
async fn mapi_over_http_folder_move_copy_rejects_event_destination_without_side_effects() {
    let source_id = Uuid::parse_str("35353535-3535-4535-8535-353535353537").unwrap();
    let event_id = Uuid::parse_str("37373737-3737-4737-8737-373737373739").unwrap();
    let source_mapi_id = test_mapi_uuid_id(&source_id);
    ...
    let store = FakeStore {
        ...
        events: Arc::new(Mutex::new(vec![AccessibleEvent {
            ...
            title: "Not a destination folder".to_string(),
            location: String::new(),
        ...
        mapi_identities: Arc::new(Mutex::new(HashMap::from([
            (source_id, source_mapi_id),
            (event_id, event_mapi_id),
        ]))),
        updated_mailboxes: updated_mailboxes.clone(),
        created_mailboxes: created_mailboxes.clone(),
        destroyed_mailboxes: destroyed_mailboxes.clone(),
        deleted_emails: deleted_emails.clone(),
        deleted_events: deleted_events.clone(),
        ..Default::default()
    };
    ...
    for rop_id in [0x35u8, 0x36] {
        let rops = crate::mapi::identity::with_current_mapi_identity_codec(
            ...
            async {
                ...
                append_rop_open_message(&mut rops, 2, 3, calendar_folder_id, event_mapi_id);
                rops.extend_from_slice(&[rop_id, 0x00, 0x01, 0x03, 0x00]);
                if rop_id == 0x36 {
                    rops.push(0x00);
                }
                rops.push(0x01);
        ...
        assert!(contains_bytes(
            &response_rops,
            &[rop_id, 0x01, 0x0F, 0x01, 0x04, 0x80, 0x00]
        ));
        renew_mapi_request_id(&mut execute_headers);
    }

    for rop_id in [0x58u8, 0x92, 0x1D, 0x35, 0x36] {
        let rops = crate::mapi::identity::with_current_mapi_identity_codec(
            identity_codec.clone(),
            async {
                let mut rops = Vec::new();
                ...
                match rop_id {
                    0x58 | 0x92 => {
                        rops.extend_from_slice(&[rop_id, 0x00, 0x03, 0x00, 0x00]);
                    }
                    0x1D => {
                        rops.extend_from_slice(&[rop_id, 0x00, 0x03, 0x00]);
                        append_mapi_wire_id(&mut rops, source_mapi_id);
                    }
                    0x35 | 0x36 => {
                        rops.extend_from_slice(&[rop_id, 0x00, 0x03, 0x01, 0x00]);
                        if rop_id == 0x36 {
                            rops.push(0x00);
                        }
                        rops.push(0x01);
                        append_mapi_wire_id(&mut rops, source_mapi_id);
                        rops.extend_from_slice(&utf16z("Wrong source object"));
                    }
                    _ => unreachable!(),
                }
                rops
        ...
        assert!(contains_bytes(
            &response_rops,
            &[rop_id, 0x03, 0x02, 0x01, 0x04, 0x80, 0x00]
        ));
        renew_mapi_request_id(&mut execute_headers);
    }
...
#[tokio::test]
async fn mapi_over_http_microsoft_folder_move_accepts_nonzero_boolean_fields_and_copy_rejects() {
    ...
    for (rop_id, booleans, name) in [
        ...
        let response = service
            ...
            .unwrap();
        let response_rops = response_rops_from_execute_response(response).await;
        let expected_response = if rop_id == 0x35 {
            vec![rop_id, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00]
        } else {
...
async fn mapi_over_http_ipm_subtree_ost_identity_survives_reconnect() {
    ...
    let (execute_headers, reconnect_logon_handle) = mapi_connect_with_private_logon(&service).await;
...
async fn mapi_over_http_open_folder_defers_property_projection_until_getprops() {
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&open_rops, &[logon_handle])),
        )
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
...
async fn mapi_over_http_additional_ren_entry_ids_canonicalize_reserved_slots_across_reconnect() {
    ...
    let first_reserved_counter = store
        ...
        .unwrap();
    let service = ExchangeService::new(store);
    let (first_headers, logon_handle) = mapi_connect_with_private_logon(&service).await;
    ...
    let set_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &first_headers,
            &execute_body(&rop_buffer(&set_rops, &[logon_handle])),
        )
        .await
        .unwrap();
    ...
    let root_set_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(&root_set_rops, &[logon_handle])),
        )
        .await
        .unwrap();
    let mut truncated_set_headers = mapi_headers("Execute");
    ...
    let truncated_set_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &truncated_set_headers,
            &execute_body(&rop_buffer(&truncated_set_rops, &[logon_handle])),
        )
        .await
        .unwrap();
    ...
    let (mut reconnect_headers, reconnect_logon_handle) =
    ...
    let get_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &reconnect_headers,
            &execute_body(&rop_buffer(&get_rops, &[reconnect_logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    ...
    let get_all_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &get_all_rops,
                &[reconnect_logon_handle, u32::MAX],
            )),
        )
        .await
        .unwrap();
...
async fn mapi_over_http_replayed_empty_folder_request_id_does_not_repeat_delete() {
    ...
    rops.push(0);
    rops.extend_from_slice(&[0x58, 0x00, 0x01, 0x00, 0x00]);
    let body = execute_body(&rop_buffer(&rops, &[logon_handle, u32::MAX]));
...
async fn mapi_over_http_open_folder_accepts_additional_ren_junk_alias_in_a_new_session() {
    ...
    let (open_headers, second_logon_handle) = mapi_connect_with_private_logon(&service).await;
    let mut open_rops = Vec::new();
...
async fn mapi_over_http_scalar_calendar_entry_id_alias_survives_a_new_session() {
    ...
    let (second_headers, second_logon_handle) = mapi_connect_with_private_logon(&service).await;
    let mut open_rops = Vec::new();
