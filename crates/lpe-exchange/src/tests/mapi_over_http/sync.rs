...
async fn mapi_over_http_connect_reestablishes_session_context_with_open_sync_handle() {
    ...
    let first_cookie = execute_headers
        .get("cookie")
        .unwrap()
        .to_str()
        .unwrap()
    ...
    let configure_request = execute_body(&rop_buffer(
        &configure_rops,
    ...
    let reconnected_cookie = mapi_cookie_header(&reconnect);
...
async fn mapi_over_http_move_folder_updates_custom_canonical_mailbox_and_hierarchy_sync() {
    ...
    let (hierarchy_headers, hierarchy_logon_handle) =
    ...
    let hierarchy_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &hierarchy_rops,
                &[hierarchy_logon_handle, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
...
async fn mapi_over_http_microsoft_oxcmsg_save_message_keep_open_read_write_imports_canonical_email()
{
    ...
    // [MS-OXCMSG] sections 2.2.3.3.1 and 4.9 preserve the Message for
    // KeepOpenReadWrite only until RopRelease; [MS-OXCROPS]
    // section 3.2.5.4 requires ecInvalidObject for that released handle.
    assert!(
        contains_bytes(response_rops, &[0x07, 0x02, 0x08, 0x01, 0x04, 0x80]),
...
async fn mapi_over_http_chain_packs_terminal_fast_transfer_get_buffer_until_done() {
    ...
    let mut payload = rop_buffer(&rops, &[logon_handle, u32::MAX, u32::MAX]);
...
#[tokio::test]
async fn mapi_over_http_outlook_type_five_calendar_header_ics_import_roundtrips() {
    // Outlook 16.0.20228 trace 202608111835 requests :98/:109/:110 import the
    // reserved MID 0x045f into Common Views, issue three SetProperties ROPs,
    // then SaveChangesMessage/GetPropertiesSpecific. Type 4 remains the
    // [MS-OXOCFG] section 2.2.9.5 group-header value; type 5 is the bounded
    // Calendar product variant whose complete traced lifecycle is replayed.
    let account = FakeStore::account();
    let store = FakeStore {
        session: Some(account.clone()),
        ..Default::default()
    };
    *store.next_mapi_global_counter.lock().unwrap() = 0x0400;
    let reservation_start = store
        .reserve_mapi_local_replica_ids(account.account_id, 0x0100)
        .await
        .unwrap();
    assert_eq!(reservation_start, 0x0400);
    let imported_message_id = crate::mapi::identity::mapi_store_id(0x045f);
    let imported_source_key = loaded_mapi_identity_codec(&store)
        .await
        .source_key_for_object_id(imported_message_id)
        .unwrap();
    let imported_change_key = vec![
        0xE0, 0x60, 0xAE, 0x08, 0x9D, 0xF0, 0xBF, 0x40, 0xAD, 0x58, 0x02, 0xE0, 0x86, 0x72, 0xC6,
        0x88, 0x00, 0x00, 0x04, 0x2C,
    ];
    ...
    let imported_last_modification_time = 134_309_394_439_860_000u64;
    let shortcuts = store.navigation_shortcuts.clone();
    let service = ExchangeService::new(store.clone());
    ...
    assert!(
        ...
        "ImportMessageChange must remain pending until SaveChangesMessage"
    );

    let group_id_bytes = [
        0xd1, 0x4c, 0x62, 0x6c, 0x9a, 0x9c, 0x65, 0x41, 0xa0, 0x7e, 0xdb, 0xa3, 0xea, 0x91, 0x28,
        0x15,
    ];
    let calendar_folder_type = [
        0x02, 0x78, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    let traced_search_key = [
        0xC4, 0xFC, 0x27, 0x6B, 0xD9, 0xC2, 0xD8, 0x46, 0xA7, 0x4C, 0xED, 0x26, 0xF6, 0x8D, 0xBC,
        0x34,
    ];
    ...
    append_mapi_i32_property(&mut first_batch, 0x0036_0003, 0); // PidTagSensitivity.
    append_mapi_i32_property(&mut first_batch, PID_TAG_MESSAGE_FLAGS, 73);
    append_mapi_i64_property(
        &mut first_batch,
        0x3007_0040, // PidTagCreationTime.
        imported_last_modification_time as i64,
    );
    append_mapi_binary_property(&mut first_batch, 0x300B_0102, &traced_search_key);
    append_mapi_utf16_property(&mut first_batch, 0x3FFA_001F, "test@l-p-e.ch");
    append_mapi_binary_property(
        &mut first_batch,
        PID_TAG_WLINK_GROUP_HEADER_ID,
        &group_id_bytes,
    );
    append_mapi_i32_property(&mut first_batch, 0x6847_0003, -82_773_232);
    append_mapi_i32_property(&mut first_batch, PID_TAG_WLINK_TYPE, 5);
    append_mapi_i32_property(&mut first_batch, 0x684A_0003, 0);
    append_mapi_binary_property(&mut first_batch, PID_TAG_WLINK_ORDINAL, &[0xBF]);
    append_mapi_binary_property(&mut first_batch, 0x684F_0102, &calendar_folder_type);
    append_mapi_i32_property(&mut first_batch, 0x6852_0003, 3);
    append_mapi_i32_property(&mut first_batch, 0x6853_0003, -1); // CalendarColor.
    append_mapi_i32_property(&mut first_batch, 0x6892_0003, 2); // ROGroupType.
    append_mapi_bool_property(&mut first_batch, 0x0E1F_000B, true);
    append_mapi_i64_property(
        &mut first_batch,
        PID_TAG_LAST_MODIFICATION_TIME,
        imported_last_modification_time as i64,
    ...
    append_mapi_bool_property(&mut second_batch, 0x0E1F_000B, true);
    let mut third_batch = Vec::new();
    append_mapi_utf16_property(&mut third_batch, 0x003D_001F, "");
    append_mapi_utf16_property(&mut third_batch, PID_TAG_NORMALIZED_SUBJECT_W, "Team: test");

    let mut save_rops = Vec::new();
    append_rop_set_properties(&mut save_rops, 1, 18, &first_batch);
    append_rop_set_properties(&mut save_rops, 1, 1, &second_batch);
    append_rop_set_properties(&mut save_rops, 1, 2, &third_batch);
    append_rop_save_changes_message_with_flags(&mut save_rops, 0, 1, 0x08);
    save_rops.extend_from_slice(&[0x07, 0x00, 0x01]); // GetPropertiesSpecific.
    ...
    assert_eq!(
        save_response
            ...
            .count(),
        3,
        "the three traced SetProperties calls must succeed"
    );
    assert!(
        contains_bytes(&save_response, &[0x0C, 0x00, 0, 0, 0, 0]),
        "Outlook's type-5 Calendar header must save: {save_response:02x?}"
    );
    ...
    assert!(contains_bytes(&save_response, &expected_get_properties));

    let stored = shortcuts.lock().unwrap().clone();
    assert_eq!(stored.len(), 1);
    assert_eq!(stored[0].subject, "Team: test");
    assert_eq!(stored[0].shortcut_type, 5);
    assert_eq!(stored[0].save_stamp, 0xFB10_FB10);
    assert_eq!(stored[0].section, 3);
    assert_eq!(stored[0].ordinal, vec![0xBF]);
    assert_eq!(
        stored[0].group_header_id,
        Some(Uuid::from_bytes(group_id_bytes))
    );
    assert_eq!(stored[0].target_folder_id, None);
    assert_eq!(stored[0].client_properties.calendar_color, Some(-1));
    assert_eq!(stored[0].client_properties.ro_group_type, Some(2));

    let reloaded_snapshot = store
        .load_mapi_mail_store(account.account_id, 500)
        .await
        .unwrap();
    let reloaded_message = reloaded_snapshot
        .navigation_shortcut_messages()
        .into_iter()
        .find(|message| message.canonical_id == stored[0].id)
        .expect("reloaded imported WLink");
    let durable_identity = reloaded_message
        .durable_identity
        .expect("reloaded imported WLink identity");
    assert_eq!(durable_identity.object_id, imported_message_id);
    assert_eq!(durable_identity.source_key, imported_source_key);
    assert_eq!(durable_identity.change_key, imported_change_key);
    assert_eq!(
        durable_identity.predecessor_change_list,
        imported_predecessor_change_list
    );
    assert_eq!(
        durable_identity.last_modification_time,
        imported_last_modification_time
    );
    let reloaded = store
        .fetch_mapi_navigation_shortcuts(account.account_id)
        .await
        .unwrap();
    ...
    assert!(
        strict_replguid_globset_contains_counter(
            cnset_seen_fai,
            &globcnt_bytes(durable_identity.change_number),
        )
        .unwrap(),
        "the applied Common Views import must advance CnsetSeenFAI"
    );
    assert!(
        !strict_replguid_globset_contains_counter(cnset_seen_fai, &globcnt_bytes(0x045f)).unwrap(),
        "CnsetSeenFAI contains the server CN, not the imported MID counter"
    );
    assert!(
        !strict_replguid_globset_contains_counter(
            mapi_binary_property_value(upload_state, META_TAG_CNSET_SEEN),
            &globcnt_bytes(durable_identity.change_number),
        )
        .unwrap(),
    ...
    let downloaded = sync
        .message_changes
        .iter()
        .find(|message| message.subject == "Team: test")
        .expect("reloaded type-5 Calendar header in Common Views FAI sync");
    assert!(downloaded.associated);
    assert_eq!(downloaded.mid, Some(imported_message_id));
    assert_eq!(downloaded.source_key, durable_identity.source_key);
    assert_eq!(
        downloaded.change_number,
        Some(durable_identity.change_number)
    );
    assert_eq!(downloaded.change_key, durable_identity.change_key);
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(imported_last_modification_time)
    );
    for property_tag in [
        PID_TAG_WLINK_GROUP_HEADER_ID,
        0x6847_0003,
        ...
        assert!(
            ...
            "reloaded FAI projection omitted property 0x{property_tag:08x}"
        );
    }
    assert!(!downloaded.body_tags.contains(&PID_TAG_WLINK_ENTRY_ID));
    ...
    *store.next_mapi_global_counter.lock().unwrap() = 0x0200_AF;
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&sync_rops, &[logon_handle, u32::MAX, u32::MAX])),
        )
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
...
#[tokio::test]
async fn mapi_over_http_tell_version_fast_transfer_source_and_destination_object_contract() {
    ...
    rops.extend_from_slice(&[
        0x4D, 0x00, 0x02, 0x03, // RopFastTransferSourceCopyTo
        0x00, // Level: include subobjects.
        0x00, 0x00, 0x00, 0x00, // CopyFlags
        0x03, // SendOptions: ForUpload.
        0x00, 0x00, // PropertyTagCount
        0x86, 0x00, 0x03, // RopTellVersion
    ]);
    rops.extend_from_slice(&[15, 20, 0, 1, 0, 0]);
    append_rop_create_message(&mut rops, 1, 4, folder_id);
    rops.extend_from_slice(&[
        0x53, 0x00, 0x04, 0x05, 0x01, 0x00, // FastTransfer DestinationConfigure.
        0x86, 0x00, 0x05, // RopTellVersion on the upload context.
    ]);
    rops.extend_from_slice(&[15, 20, 0, 1, 0, 0]);
    rops.extend_from_slice(&[
    ...
    assert!(contains_bytes(&response_rops, &[0x86, 0x03, 0, 0, 0, 0]));
    assert!(contains_bytes(&response_rops, &[0x53, 0x05, 0, 0, 0, 0]));
    assert!(contains_bytes(&response_rops, &[0x86, 0x05, 0, 0, 0, 0]));
    let chunks = mapi_fast_transfer_chunks(&response_rops);
    assert_eq!(chunks.len(), 1, "{response_rops:02x?}");
    assert!(contains_bytes(
        &chunks[0].1,
        &utf16z("TellVersion sync message")
    ));
}

#[tokio::test]
async fn mapi_over_http_tell_version_ics_object_contract_rejects_source_and_collector() {
    let message_id = "41404040-4040-4040-4040-404040404041";
    let mailbox_id = "55555555-5555-5555-5555-555555555555";
    let mut inbox = FakeStore::mailbox(mailbox_id, "inbox", "Inbox");
    inbox.total_emails = 1;
    let email = FakeStore::email(message_id, mailbox_id, "inbox", "ICS remains aligned");
    let store = FakeStore {
        session: Some(FakeStore::account()),
        mailboxes: Arc::new(Mutex::new(vec![inbox])),
        emails: Arc::new(Mutex::new(vec![email])),
    ...
    rops.extend_from_slice(&[
        0x70, 0x00, 0x01, 0x02, // RopSynchronizationConfigure
        0x01, 0x00, 0x00, 0x00, // Content synchronization.
        0x00, 0x00, // RestrictionDataSize
        0x00, 0x00, 0x00, 0x00, // SynchronizationExtraFlags
        0x00, 0x00, // PropertyTagCount
        0x7E, 0x00, 0x01, 0x03, 0x01, // RopSynchronizationOpenCollector, contents.
        0x86, 0x00, 0x02, // RopTellVersion on ICS download context.
    ]);
    rops.extend_from_slice(&[15, 20, 0, 1, 0, 0]);
    rops.extend_from_slice(&[
        0x86, 0x00, 0x03, // RopTellVersion on ICS upload context.
    ]);
    rops.extend_from_slice(&[15, 20, 0, 1, 0, 0]);
    rops.extend_from_slice(&[
        0x82, 0x00, 0x03, 0x04, // Collector remains valid for GetTransferState.
        0x4E, 0x00, 0x02, // Valid ICS GetBuffer after both errors.
    ]);
    rops.extend_from_slice(&4096u16.to_le_bytes());
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let response_rops = response_rops_from_execute_response(response).await;
    for response_handle_index in [0x02, 0x03] {
        let expected = [0x86, response_handle_index, 0x02, 0x01, 0x04, 0x80];
        assert_eq!(
            response_rops
                ...
                .count(),
            1,
            "TellVersion wrong-context response was missing or misaligned: {response_rops:02x?}",
        );
    }
    assert!(contains_bytes(&response_rops, &[0x82, 0x04, 0, 0, 0, 0]));
    let chunks = mapi_fast_transfer_chunks(&response_rops);
    assert_eq!(chunks.len(), 1, "{response_rops:02x?}");
    assert!(contains_bytes(&chunks[0].1, b"ICS remains aligned"));
}
...
async fn mapi_over_http_sync_configure_separates_content_and_hierarchy_manifests() {
    ...
    let content_request = execute_body(&rop_buffer(
        &content_rops,
        &[logon_handle, u32::MAX, u32::MAX],
    ));
    ...
    renew_mapi_request_id(&mut execute_headers);
    let hierarchy_request = execute_body(&rop_buffer(
        &hierarchy_rops,
        &[logon_handle, u32::MAX, u32::MAX],
    ));
    let hierarchy_response = service
...
#[tokio::test]
async fn mapi_over_http_inbox_additional_ren_entry_ids_versions_and_replays_hierarchy_in_postgresql(
) -> anyhow::Result<()> {
    // [MS-OXOSFLD] section 2.2.4 defines the five reserved entries in
    // PidTagAdditionalRenEntryIds. Exercise the complete wire-to-PostgreSQL
    // lane because FakeStore cannot couple profile writes, hierarchy versions,
    // journal rows, and later stateful ICS downloads in one transaction.
    let Some(fixture) = postgres_mapi_calendar_fixture().await? else {
        return Ok(());
    };
    let storage = fixture.storage.clone();
    ...
    let inbox_mailbox = mailboxes
        ...
        .clone();
    let identity_codec =
        crate::mapi::load_mapi_identity_codec_for_test(&storage, fixture.account_id).await?;
    let inbox_source_key = identity_codec
        .source_key_for_object_id(crate::mapi::identity::INBOX_FOLDER_ID)
        .expect("durable Inbox SourceKey");
    ...
    .map(|folder_id| {
        identity_codec
            .folder_entry_id_from_object_id(fixture.account_id, folder_id)
            .expect("special-folder EntryID")
    })
    .collect::<Vec<_>>();
    let profile_value_for = |entries: &[Vec<u8>]| {
        let entries = entries.iter().map(Vec::as_slice).collect::<Vec<_>>();
        let mut tagged_value = Vec::new();
    ...
    append_rop_outlook_hierarchy_sync_manifest_get_buffer(&mut initial_rops, 1, 2, 31_680);
    ...
    let initial_response_rops = response_rops_from_execute_response(initial_response).await;
    let initial_hierarchy = strict_hierarchy_sync_transfer_from_response(&initial_response_rops)
        .map_err(anyhow::Error::msg)?;
    let initial_inbox = initial_hierarchy
        .folder_changes
        .iter()
        .find(|folder| folder.source_key == inbox_source_key)
        .expect("initial Inbox folderChange");
    assert_eq!(
        initial_inbox.additional_ren_entry_ids.as_ref(),
    ...
    // while preserving the opaque value at index 5.
    let empty = &[][..];
    let root_tail = [0x4d, 0x3c, 0x2b, 0x1a];
    let mut root_property_values = Vec::new();
    append_mapi_multi_binary_property(
        &mut root_property_values,
        PID_TAG_ADDITIONAL_REN_ENTRY_IDS,
        &[empty, empty, empty, empty, empty, &root_tail],
    );
    let mut expected_root_entries = canonical_entries.clone();
    ...
    assert!(
        contains_bytes(&root_set_response_rops, &[0x0A, 0x01, 0, 0, 0, 0, 0, 0]),
        "Root AdditionalRenEntryIds SetProperties failed: {root_set_response_rops:02x?}"
    );

    let after_root_identity = storage
        .fetch_or_allocate_mapi_identities(fixture.account_id, &inbox_identity_request)
        .await?
        .remove(0);
    assert_ne!(
        after_root_identity.change_number, initial_identity.change_number,
        "Root SetProperties must rotate the durable Inbox CN"
    );
    assert_ne!(
        after_root_identity.change_key, initial_identity.change_key,
        "Root SetProperties must rotate the durable Inbox ChangeKey"
    );
    assert!(
        after_root_identity.last_modification_time > initial_identity.last_modification_time,
        "Root SetProperties must advance the durable Inbox LMT"
    );
    assert!(test_mapi_pcl_includes_change_key(
    ...
    let stored_root_profile_value = sqlx::query_scalar::<_, Vec<u8>>(
        r#"
        SELECT property_value
        FROM mapi_folder_profile_property_values
        WHERE account_id = $1
    ...
    .fetch_one(storage.pool())
    .await?;
    assert_eq!(stored_root_profile_value, expected_root_profile_value);
    let root_journal_rows = sqlx::query_as::<_, (i64, serde_json::Value)>(
        r#"
    ...
    .bind(initial_journal_cursor)
    .fetch_all(storage.pool())
    .await?;
    assert_eq!(
        root_journal_rows.len(),
        1,
        "one Root property commit must write exactly one Inbox hierarchy journal row"
    );
    assert_eq!(
        root_journal_rows[0].1["mapiFolderId"],
        serde_json::Value::String(after_root_identity.object_id.to_string())
    );
    assert_eq!(
        root_journal_rows[0].1["mapiChangeNumber"],
        serde_json::Value::String(after_root_identity.change_number.to_string())
    );

    let mut first_replay_rops = Vec::new();
    append_rop_open_folder(
        &mut first_replay_rops,
        0,
        1,
        crate::mapi::identity::IPM_SUBTREE_FOLDER_ID,
    );
    ...
    renew_mapi_request_id(&mut execute_headers);
    let first_replay_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(
        ...
        .await?;
    let first_replay_response_rops =
        response_rops_from_execute_response(first_replay_response).await;
    let first_replay = strict_hierarchy_sync_transfer_from_response(&first_replay_response_rops)
        .map_err(anyhow::Error::msg)?;
    assert_eq!(
        first_replay.folder_changes.len(),
        1,
        "the one committed hierarchy journal row must replay exactly Inbox"
    );
    let replayed_root_inbox = &first_replay.folder_changes[0];
    assert_eq!(replayed_root_inbox.source_key, inbox_source_key);
    assert_eq!(
        replayed_root_inbox.additional_ren_entry_ids.as_ref(),
        Some(&expected_root_entries)
    );
    assert_eq!(
        replayed_root_inbox.change_key,
        after_root_identity.change_key
    );
    assert_eq!(
        replayed_root_inbox.predecessor_change_list,
        after_root_identity.predecessor_change_list
    );
    assert_eq!(
        replayed_root_inbox.last_modification_time,
        after_root_identity.last_modification_time
    );
    assert!(strict_replguid_globset_contains_counter(
        &first_replay.cnset_seen,
        &globcnt_bytes(after_root_identity.change_number),
    )
    .map_err(anyhow::Error::msg)?);
    ...
    append_mapi_multi_binary_property(
        &mut import_property_values,
        PID_TAG_ADDITIONAL_REN_ENTRY_IDS,
        &[empty, empty, empty, empty, empty, &import_tail],
    );
    // Exchange 2016 raw/249 appends this bounded reserved record. Preserve it
    // through the same hierarchy-import transaction as 36D8.
    let imported_extended_folder_flags = vec![
        0x01, 0x04, 0x00, 0x00, 0x10, 0x00, 0x06, 0x04, 0x03, 0x00, 0x00, 0x00,
    ];
    append_mapi_binary_property(
        &mut import_property_values,
        PID_TAG_EXTENDED_FOLDER_FLAGS,
        &imported_extended_folder_flags,
    );
    let mut expected_import_entries = canonical_entries.clone();
    expected_import_entries.push(import_tail.to_vec());
    let expected_import_profile_value = profile_value_for(&expected_import_entries);
    ...
    import_rops.extend_from_slice(&2u16.to_le_bytes());
    import_rops.extend_from_slice(&import_property_values);
    import_rops.extend_from_slice(&[
        0x82, 0x00, 0x02, 0x03, // GetTransferState.
        0x4E, 0x00, 0x03, // GetBuffer.
    ]);
    import_rops.extend_from_slice(&8192u16.to_le_bytes());
    renew_mapi_request_id(&mut execute_headers);
    ...
    assert!(
        contains_bytes(&import_response_rops, &[0x73, 0x02, 0, 0, 0, 0]),
        "existing-Inbox ImportHierarchyChange failed: {import_response_rops:02x?}"
    );

    let after_import_identity = storage
        .fetch_or_allocate_mapi_identities(fixture.account_id, &inbox_identity_request)
        .await?
        .remove(0);
    assert_ne!(
    ...
    .map_err(anyhow::Error::msg)?);
    let imported_profile_tags = vec![
        i64::from(PID_TAG_ADDITIONAL_REN_ENTRY_IDS),
        i64::from(PID_TAG_EXTENDED_FOLDER_FLAGS),
    ];
    let stored_import_profile_values = sqlx::query_as::<_, (i64, i32, Vec<u8>)>(
        r#"
        SELECT property_tag, property_type, property_value
        FROM mapi_folder_profile_property_values
        WHERE account_id = $1
          AND folder_id = $2
          AND property_tag = ANY($3)
        ORDER BY property_tag
        "#,
    )
    .bind(fixture.account_id)
    .bind(crate::mapi::identity::INBOX_FOLDER_ID as i64)
    .bind(&imported_profile_tags)
    .fetch_all(storage.pool())
    .await?;
    assert_eq!(
        stored_import_profile_values,
        vec![
            (
                i64::from(PID_TAG_ADDITIONAL_REN_ENTRY_IDS),
                0x1102,
        ...
        "ImportHierarchyChange must atomically preserve every supported profile property"
    );
    let import_journal_rows = sqlx::query_as::<_, (i64, serde_json::Value)>(
        r#"
        SELECT cursor, summary_json
        FROM mail_change_log
        WHERE account_id = $1
    ...
    .await?;
    assert_eq!(
        import_journal_rows.len(),
        1,
        "one imported property/version commit must write exactly one Inbox hierarchy journal row"
    );
    assert_eq!(
        import_journal_rows[0].1["mapiFolderId"],
        serde_json::Value::String(after_import_identity.object_id.to_string())
    );
    assert_eq!(
        import_journal_rows[0].1["mapiChangeNumber"],
        serde_json::Value::String(after_import_identity.change_number.to_string())
    );

    let mut second_replay_rops = Vec::new();
    append_rop_open_folder(
        &mut second_replay_rops,
        0,
        1,
        crate::mapi::identity::IPM_SUBTREE_FOLDER_ID,
    );
    append_rop_outlook_hierarchy_sync_manifest_get_buffer_with_state(
        ...
        2,
        31_680,
        &first_replay.idset_given,
        &first_replay.cnset_seen,
    );
    renew_mapi_request_id(&mut execute_headers);
    let second_replay_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(
        ...
        .await?;
    let second_replay_response_rops =
        response_rops_from_execute_response(second_replay_response).await;
    let second_replay = strict_hierarchy_sync_transfer_from_response(&second_replay_response_rops)
        .map_err(anyhow::Error::msg)?;
    assert_eq!(second_replay.folder_changes.len(), 1);
    let replayed_imported_inbox = &second_replay.folder_changes[0];
    assert_eq!(replayed_imported_inbox.source_key, inbox_source_key);
    assert_eq!(
        replayed_imported_inbox.additional_ren_entry_ids.as_ref(),
        Some(&expected_import_entries)
    );
    assert_eq!(
        replayed_imported_inbox.change_key,
        after_import_identity.change_key
    );
    assert_eq!(
        replayed_imported_inbox.predecessor_change_list,
        after_import_identity.predecessor_change_list
    );
    assert_eq!(
        replayed_imported_inbox.last_modification_time,
        after_import_identity.last_modification_time
    );
    assert!(strict_replguid_globset_contains_counter(
        &second_replay.cnset_seen,
        &globcnt_bytes(after_import_identity.change_number),
    )
    .map_err(anyhow::Error::msg)?);
...
async fn mapi_over_http_hierarchy_sync_preserves_nested_folder_parent_keys() {
    ...
    let (child_scope_headers, child_scope_logon_handle) =
        mapi_connect_with_private_logon(&service).await;
    ...
    let response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &child_scope_rops,
                &[child_scope_logon_handle, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn mapi_over_http_hierarchy_sync_replays_version_3_server_checkpoint() {
    let inbox_id = Uuid::parse_str("55555555-5555-5555-5555-555555555555").unwrap();
    let mut inbox = FakeStore::mailbox(&inbox_id.to_string(), "inbox", "Inbox");
    inbox.total_emails = 3;
    inbox.unread_emails = 1;
...
async fn mapi_over_http_content_sync_after_empty_folder_advances_empty_final_state() {
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&purge_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let response_rops = response_rops_from_execute_response(response).await;
...
async fn mapi_over_http_upload_import_collector_handles_never_advance_download_checkpoints() {
    ...
    let imported_source_key = mapi_mailstore::source_key_for_store_id(imported_message_id);
    let mut values = Vec::new();
    append_mapi_binary_property(&mut values, PID_TAG_SOURCE_KEY, &imported_source_key);
    ...
    rops.extend_from_slice(&[
        ...
        0x82, 0x00, 0x02, 0x04, // RopSynchronizationGetTransferState from collector
    ...
    assert!(
        contains_bytes(&response_rops, &[0x82, 0x04, 0, 0, 0, 0]),
        "GetTransferState failed: {response_rops:02x?}"
    );
    assert!(!contains_bytes(&response_rops, state));
    assert_eq!(store.imported_emails.lock().unwrap().len(), 1);
    let imported_email = store.emails.lock().unwrap().last().unwrap().clone();
    let imported_change_number = identity_change_numbers.lock().unwrap()[&imported_email.id];
...
async fn mapi_over_http_associated_message_uploads_do_not_create_visible_items() {
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&inbox_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let response_rops = response_rops_from_execute_response(response).await;
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&calendar_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let response_rops = response_rops_from_execute_response(response).await;
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&freebusy_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let response_rops = response_rops_from_execute_response(response).await;
...
async fn mapi_over_http_sync_import_associated_message_persists_and_replays_fai() {
    ...
    *store.next_mapi_global_counter.lock().unwrap() =
    ...
    let (mut sync_headers, sync_logon_handle) = mapi_connect_with_private_logon(&service).await;
    ...
    let sync_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &sync_rops,
                &[sync_logon_handle, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
    ...
    renew_mapi_request_id(&mut sync_headers);
    ...
    *store.next_mapi_global_counter.lock().unwrap() = 0x023e;
    ...
    let fai_watermark_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&fai_watermark_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    ...
    let (mut reopen_headers, reopen_logon_handle) = mapi_connect_with_private_logon(&service).await;
    ...
    let reopen_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &reopen_rops,
                &[reopen_logon_handle, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
    ...
    // 202607221217.
    for attempt in 1..8 {
        let (repeated_headers, repeated_logon_handle) =
            mapi_connect_with_private_logon(&service).await;
        ...
        let response = service
            .handle_mapi(
                ...
                &execute_body(&rop_buffer(
                    &repeated_rops,
                    &[repeated_logon_handle, u32::MAX, u32::MAX, u32::MAX],
                )),
            )
            .await
            .unwrap();
...
async fn mapi_over_http_microsoft_oxocfg_configuration_examples_round_trip_fai() {
    ...
    let (mut table_headers, table_logon_handle) = mapi_connect_with_private_logon(&service).await;
    ...
    let table_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &table_rops,
                &[table_logon_handle, u32::MAX, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
    ...
    renew_mapi_request_id(&mut table_headers);
    let open_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &open_rops,
                &[table_logon_handle, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
...
async fn mapi_over_http_open_associated_message_by_imported_source_key_id() {
    ...
    let service = ExchangeService::new(store);
    let (open_headers, logon_handle) = mapi_connect_with_private_logon(&service).await;
...
async fn mapi_over_http_replays_outlook_calendar_sync_import_then_save() {
    ...
    assert_eq!(committed_change_key, change_xid);
    let committed_lmt_offset = get_properties_row + 3 + change_key_size;
    let committed_last_modification_time = i64::from_le_bytes(
        save_response[committed_lmt_offset..committed_lmt_offset + 8]
            .try_into()
            .unwrap(),
    );
    let committed_local_commit_time = i64::from_le_bytes(
        save_response[committed_lmt_offset + 8..committed_lmt_offset + 16]
            .try_into()
            .unwrap(),
    );
    assert_eq!(
        committed_last_modification_time, imported_last_modification_time,
        "Save/GetProps must retain the imported Calendar version LMT"
    );
    assert_ne!(
        committed_local_commit_time, imported_last_modification_time,
        "Save/GetProps must keep the local commit time separate from the imported version LMT"
    );
    ...
    assert_eq!(
        canonical_version.last_modification_time,
        imported_last_modification_time as u64
    );
    ...
    full_sync_store
        ...
        .insert(
            ...
            MapiEventVersion {
                ...
                .unwrap(),
                last_modification_time: test_filetime("2026-08-11", "12:12") as u64,
    ...
    assert_eq!(
        downloaded.last_modification_time,
        Some(test_filetime("2026-08-11", "12:12") as u64),
        "fresh Calendar ICS must expose the direct web version LMT"
    );
    assert!(test_mapi_pcl_includes_change_key(
        &downloaded.predecessor_change_list,
        &change_xid
...
async fn mapi_over_http_replays_outlook_calendar_import_move_to_deleted_items() {
    ...
    let store = FakeStore {
        ...
        mapi_event_identity_versions: Arc::new(Mutex::new(HashMap::from([(
            ...
            MapiEventVersion {
                ...
                last_modification_time: test_filetime("2026-07-16", "20:52") as u64,
    ...
    let response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &import_move_rops,
                &[logon_handle, collector_handles[2]],
            )),
        )
        .await
        .unwrap();
...
async fn mapi_over_http_replays_outlook_calendar_move_then_modifies_deleted_event() {
    ...
    let response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &update_rops,
                &[logon_handle, collector_handles[2], u32::MAX],
            )),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        updated_version.last_modification_time,
        imported_last_modification_time as u64
    );
}
...
async fn execute_existing_calendar_sync_import(
    ...
    renew_mapi_request_id(&mut execute_headers);
    let mut request_handles = vec![logon_handle, collector_handles[2], u32::MAX];
    if request_transfer_state {
...
async fn mapi_over_http_calendar_sync_import_conflict_merges_both_predecessor_lists() {
    ...
    for predecessor in [&server_change_key, &client_change_key] {
        assert!(test_mapi_pcl_includes_change_key(
...
async fn mapi_over_http_calendar_sync_import_conflict_keeps_the_newer_server_content() {
    ...
    assert_eq!(
        version.last_modification_time,
        test_filetime("2026-07-17", "10:00") as u64,
        "a server-winning conflict must retain the current version LMT"
    );
    for predecessor in [&server_change_key, &client_change_key] {
        assert!(test_mapi_pcl_includes_change_key(
            &version.predecessor_change_list,
...
async fn mapi_over_http_sync_import_delete_and_read_state_use_canonical_store() {
    ...
    assert!(contains_bytes(
        &response_rops,
        &[
            0x80, 0x02, 0, 0, 0, 0, // exact six-byte ReadState response
            0x74, 0x02, 0, 0, 0, 0, // immediately followed by ImportDeletes
        ]
    ));
...
async fn mapi_over_http_sync_import_read_state_requires_valid_handle() {
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&rops, &[logon_handle])),
        )
        .await
        .unwrap();
    ...
    assert!(contains_bytes(
        &response_rops,
        &[0x80, 0x02, 0xB9, 0x04, 0x00, 0x00]
    ));
    assert!(emails.lock().unwrap()[0].unread);
}

#[tokio::test]
async fn mapi_over_http_sync_import_read_state_requires_content_collector() {
    let message_id = "42424242-4242-4242-4242-424242424253";
    let mut email = FakeStore::email(
        message_id,
        "55555555-5555-5555-5555-555555555555",
        "inbox",
        "Read import wrong object",
    );
    email.unread = true;
    let emails = Arc::new(Mutex::new(vec![email]));
    let store = FakeStore {
    ...
    rops.extend_from_slice(&[0x02, 0x00, 0x00, 0x01]); // RopOpenFolder
    append_mapi_wire_id(&mut rops, test_mapi_folder_id(5));
    rops.push(0);
    rops.extend_from_slice(&[
        0x80, 0x00, 0x01, // ReadStateChanges against the Folder, not a collector
    ]);
    rops.extend_from_slice(&1u16.to_le_bytes());
    append_mapi_wire_id(&mut rops, test_mapi_message_id(message_id));
    rops.push(1);
    ...
    assert!(contains_bytes(
        &response_rops,
        &[0x80, 0x01, 0x02, 0x01, 0x04, 0x80]
    ));
    assert!(emails.lock().unwrap()[0].unread);
}

#[tokio::test]
async fn mapi_over_http_sync_import_read_state_preflights_before_mutation() {
    let existing_id = "42424242-4242-4242-4242-424242424251";
    let missing_id = "43434343-4343-4343-4343-434343434352";
    let mut email = FakeStore::email(
        existing_id,
        "55555555-5555-5555-5555-555555555555",
        "inbox",
        "Read import atomic preflight",
    );
    email.unread = true;
    let emails = Arc::new(Mutex::new(vec![email]));
    let store = FakeStore {
    ...
    rops.extend_from_slice(&2u16.to_le_bytes());
    for message_id in [existing_id, missing_id] {
        append_mapi_wire_id(&mut rops, test_mapi_message_id(message_id));
        rops.push(1);
    }
    ...
    let response_rops = response_rops_from_execute_response(response).await;
    assert!(
        contains_bytes(&response_rops, &[0x80, 0x02, 0x0F, 0x01, 0x04, 0x80]),
        "unexpected read-state preflight response: {response_rops:02x?}"
    );
    assert!(emails.lock().unwrap()[0].unread);
}
...
async fn mapi_over_http_sync_import_deletes_removes_common_views_wlink_by_source_key_and_reloads() {
    ...
    let mut failed_save_state_rops = vec![
        0x82, 0x00, 0x00, 0x01, // RopSynchronizationGetTransferState.
        0x4E, 0x00, 0x01, // RopFastTransferSourceGetBuffer.
    ];
    failed_save_state_rops.extend_from_slice(&4096u16.to_le_bytes());
    renew_mapi_request_id(&mut execute_headers);
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(
                &failed_save_state_rops,
                &[collector_handles[2], u32::MAX],
            )),
        )
        .await
        .unwrap();
    let failed_save_state = response_rops_from_execute_response(response).await;
    assert_content_upload_final_state_omits_given(&failed_save_state);
}
...
async fn mapi_over_http_sync_import_read_state_ignores_transient_associated_artifact() {
    ...
    let response_rops = response_rops_from_execute_response(response).await;
    assert!(contains_bytes(&response_rops, &[0x80, 0x02, 0, 0, 0, 0]));
}

#[tokio::test]
async fn mapi_over_http_sync_import_read_state_ignores_durable_fai_message() {
    let account = FakeStore::account();
    let inbox_id = Uuid::parse_str("55555555-5555-4555-9555-555555555501").unwrap();
    let config_id = Uuid::parse_str("e0fdf7ca-15f8-4c62-bf51-d543d69a1410").unwrap();
    let config_object_id = crate::mapi::identity::mapi_store_id(197_410);
    let associated_configs = Arc::new(Mutex::new(vec![crate::store::MapiAssociatedConfigRecord {
        id: config_id,
        account_id: account.account_id,
        folder_id: crate::mapi::identity::INBOX_FOLDER_ID,
        message_class: "IPM.Configuration.MessageListSettings".to_string(),
        subject: "Durable FAI read-state target".to_string(),
        properties_json: serde_json::json!({}),
    }]));
    let store = FakeStore {
        session: Some(account),
    ...
    rops.extend_from_slice(&1u16.to_le_bytes());
    append_mapi_wire_id(&mut rops, config_object_id);
    rops.push(1);
...
async fn mapi_over_http_sync_import_move_uses_canonical_store() {
    ...
    let reservation_start = store
        ...
        .await
        .unwrap();
    let destination_message_id = crate::mapi::identity::mapi_store_id(reservation_start + 0x0178);
    let destination_message_gid =
...
async fn mapi_over_http_microsoft_oxcfxics_4_1_2_hierarchy_delete_returns_transfer_state() {
    ...
    // [MS-OXCFXICS] section 2.2.3.2.4.5.1: Hierarchy (0x01) selects
    // folder deletions; HardDelete (0x02) alone selects message deletions.
    append_rop_sync_import_deletes(&mut rops, 0x02, 0x01, &[folder_mapi_id]);
    rops.extend_from_slice(&[
        0x82, 0x00, 0x02, 0x03, // RopSynchronizationGetTransferState
        0x4E, 0x00, 0x03, // RopFastTransferSourceGetBuffer
    ]);
...
async fn mapi_over_http_sync_import_hierarchy_change_accepts_existing_deleted_items() {
    ...
    let reopen_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&reopen_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
...
async fn mapi_over_http_sync_imported_junk_email_alias_remains_resident_when_canonical_is_emitted()
{
    ...
    let (first_headers, first_logon_handle) = mapi_connect_with_private_logon(&service).await;
    let mut hierarchy_values = Vec::new();
    append_mapi_binary_property(
        &mut hierarchy_values,
        PID_TAG_PARENT_SOURCE_KEY,
    ...
    let import_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &import_rops,
                &[first_logon_handle, u32::MAX, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
    ...
    let (mut second_headers, second_logon_handle) = mapi_connect_with_private_logon(&service).await;
    let mut open_rops = Vec::new();
    ...
    let open_response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &second_headers,
            &execute_body(&rop_buffer(&open_rops, &[second_logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let open_response_rops = response_rops_from_execute_response(open_response).await;
    ...
    let reimport_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &reimport_rops,
                &[second_logon_handle, u32::MAX, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
    ...
    let hierarchy_response = service
        .handle_mapi(
            ...
            &execute_body(&rop_buffer(
                &hierarchy_rops,
                &[second_logon_handle, u32::MAX, u32::MAX],
            )),
        )
        .await
        .unwrap();
...
async fn mapi_over_http_microsoft_folder_search_criteria_example_round_trips_message_class_and_importance(
    ...
    let response = service
        .handle_mapi(
            MapiEndpoint::Emsmdb,
            &execute_headers,
            &execute_body(&rop_buffer(&get_string8_rops, &[logon_handle, u32::MAX])),
        )
        .await
        .unwrap();
    let response_rops = response_rops_from_execute_response(response).await;
