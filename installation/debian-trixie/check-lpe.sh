...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_named_properties');" | grep -qx 'mapi_named_properties' \
  || fail "Table public.mapi_named_properties is missing. LPE ${expected_release_version} requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_custom_property_values');" | grep -qx 'mapi_custom_property_values' \
  || fail "Table public.mapi_custom_property_values is missing. LPE ${expected_release_version} requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Found table public.mapi_custom_property_values"

mapi_custom_object_kind_constraint_count="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT COUNT(*) FROM pg_constraint WHERE conrelid = 'public.mapi_custom_property_values'::regclass AND contype = 'c' AND pg_get_constraintdef(oid) LIKE '%public_folder_item%' AND pg_get_constraintdef(oid) LIKE '%delegate_freebusy_message%';")" \
  || fail "Unable to inspect MAPI custom property object-kind constraint"
[[ "$mapi_custom_object_kind_constraint_count" -ge "1" ]] \
  || fail "MAPI custom property object-kind constraint is incomplete. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "MAPI custom property object-kind constraint is current"

psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_profile_settings');" | grep -qx 'mapi_profile_settings' \
  || fail "Table public.mapi_profile_settings is missing. LPE ${expected_release_version} requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_folder_profile_property_values');" | grep -qx 'mapi_folder_profile_property_values' \
  || fail "Table public.mapi_folder_profile_property_values is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "${mapi_special_folder_alias_shape_ok}" == "1" ]] \
  || fail "MAPI special-folder alias schema is missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_local_replica_range_shape_status" == "1" ]] \
  || fail "MAPI local replica range table shape is missing or invalid. Initialize a fresh LPE ${expected_release_version} database with init-schema.sh."
...
[[ "$mapi_outlook_cache_fidelity_shape_status" == "1" ]] \
  || fail "MAPI WLink/configuration FAI fidelity shape is missing or invalid. Initialize a fresh LPE ${expected_release_version} database with init-schema.sh."
...
[[ "$mapi_shortcut_group_column_count" == "2" ]] \
  || fail "MAPI navigation shortcut group/header columns are missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_shortcut_target_nullable" == "YES" ]] \
  || fail "MAPI navigation shortcut target_folder_id is still NOT NULL. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_shortcut_save_stamp_column_count" == "1" && "$mapi_shortcut_save_stamp_constraint_count" == "1" ]] \
  || fail "MAPI navigation shortcut save_stamp column is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_associated_config_messages');" | grep -qx 'mapi_associated_config_messages' \
  || fail "Table public.mapi_associated_config_messages is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Found table public.mapi_associated_config_messages"

mapi_change_log_object_kind_constraint_ok="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT COUNT(*) FROM pg_constraint WHERE conrelid = 'public.mail_change_log'::regclass AND contype = 'c' AND pg_get_constraintdef(oid) LIKE '%associated_config%' AND pg_get_constraintdef(oid) LIKE '%delegate_freebusy_message%' AND pg_get_constraintdef(oid) NOT LIKE '%account_id IS NOT NULL%';")" \
  || fail "Unable to inspect the MAPI change-log object-kind constraint"
[[ "$mapi_change_log_object_kind_constraint_ok" -ge "1" ]] \
  || fail "MAPI change-log object-kind constraint is stale. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "MAPI change-log object-kind constraint is current"

mapi_change_log_object_shape_constraint_ok="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT COUNT(*) FROM pg_constraint WHERE conrelid = 'public.mail_change_log'::regclass AND contype = 'c' AND pg_get_constraintdef(oid) LIKE '%associated_config%' AND pg_get_constraintdef(oid) LIKE '%delegate_freebusy_message%' AND pg_get_constraintdef(oid) LIKE '%account_id IS NOT NULL%' AND pg_get_constraintdef(oid) LIKE '%mailbox_id IS NULL%' AND pg_get_constraintdef(oid) LIKE '%collection_id IS NULL%' AND pg_get_constraintdef(oid) LIKE '%mapiOnly%true%' AND (length(pg_get_constraintdef(oid)) - length(replace(pg_get_constraintdef(oid), 'folderId', ''))) >= 3 * length('folderId') AND (length(pg_get_constraintdef(oid)) - length(replace(pg_get_constraintdef(oid), 'mapiMessageId', ''))) >= 3 * length('mapiMessageId') AND (length(pg_get_constraintdef(oid)) - length(replace(pg_get_constraintdef(oid), 'jsonb_typeof', ''))) >= 2 * length('jsonb_typeof') AND (length(pg_get_constraintdef(oid)) - length(replace(pg_get_constraintdef(oid), '^[1-9][0-9]*$', ''))) >= 2 * length('^[1-9][0-9]*$');")" \
  || fail "Unable to inspect the MAPI change-log object-shape constraint"
[[ "$mapi_change_log_object_shape_constraint_ok" -ge "1" ]] \
  || fail "MAPI change-log object-shape constraint is stale. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "MAPI change-log object-shape constraint is current"
...
[[ "$mapi_low_dynamic_property_count" == "0" ]] \
  || fail "MAPI low dynamic named-property ids remain. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.search_folders_user_saved_name_idx');" | grep -qx 'search_folders_user_saved_name_idx' \
  || fail "User-saved Search Folder uniqueness index is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recipient_suggestions');" | grep -qx 'recipient_suggestions' \
  || fail "Table public.recipient_suggestions is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recipient_suggestions_active_email_idx');" | grep -qx 'recipient_suggestions_active_email_idx' \
  || fail "Recipient suggestions active-email uniqueness index is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recipient_suggestions_rank_idx');" | grep -qx 'recipient_suggestions_rank_idx' \
  || fail "Recipient suggestions ranking index is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recoverable_items');" | grep -qx 'recoverable_items' \
  || fail "Table public.recoverable_items is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_account_column_count" == "3" ]] \
  || fail "Recoverable-item account retention/hold columns are missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_mailbox_column_count" == "1" ]] \
  || fail "Recoverable-item mailbox retention override column is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$managed_retention_mailbox_column_count" == "1" ]] \
  || fail "Managed retention folder mailbox tag column is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$managed_retention_mailbox_fk_count" == "1" ]] \
  || fail "Managed retention folder mailbox tag constraint is missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_change_constraint_count" -ge "4" ]] \
  || fail "Recoverable-item change-log constraints are missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Recoverable-item change-log constraints are present"

recoverable_shape_constraint_ok="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT COUNT(*) FROM pg_constraint WHERE conrelid = 'public.mail_change_log'::regclass AND contype = 'c' AND pg_get_constraintdef(oid) LIKE '%sourceMailboxMessageId%' AND pg_get_constraintdef(oid) LIKE '%[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}%' AND pg_get_constraintdef(oid) NOT LIKE '%[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}%';")" \
  || fail "Unable to inspect recoverable-item replay shape constraint"
[[ "$recoverable_shape_constraint_ok" -ge "1" ]] \
  || fail "Recoverable-item replay shape constraint is stale. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$public_folder_table_count" == "6" ]] \
  || fail "Public-folder canonical tables are missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$public_folder_change_constraint_count" -ge "4" ]] \
  || fail "Public-folder change-log constraints are missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$public_folder_sync_constraint_count" == "2" ]] \
  || fail "Public-folder sync constraints are missing. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_store_identity_shape_status" == "1" ]] \
  || fail "MAPI store identity singleton or global identity constraints are missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "MAPI store identity singleton and global identity constraints are current"
...
[[ "$mapi_identity_version_column_count" == "2" ]] \
  || fail "MAPI identity version column shapes are invalid; expected bigint/bytea NOT NULL. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
calendar_event_lifecycle_column_count="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'calendar_events' AND column_name IN ('lifecycle_state', 'deleted_at', 'meeting_response_state_json', 'projection_state') AND is_nullable = CASE column_name WHEN 'deleted_at' THEN 'YES' ELSE 'NO' END AND data_type = CASE column_name WHEN 'deleted_at' THEN 'timestamp with time zone' WHEN 'meeting_response_state_json' THEN 'jsonb' ELSE 'text' END;")" \
  || fail "Unable to inspect Calendar Event lifecycle and meeting-response replay column shapes"
[[ "$calendar_event_lifecycle_column_count" == "4" ]] \
  || fail "Calendar Event Deleted Items lifecycle or meeting-response replay columns are missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Calendar Event Deleted Items lifecycle and meeting-response replay columns are current"

calendar_meeting_response_state_shape_status="$(calendar_meeting_response_state_shape_ok "$DATABASE_URL")" \
  || fail "Unable to inspect the Calendar meeting-response replay default and constraint"
[[ "$calendar_meeting_response_state_shape_status" == "1" ]] \
  || fail "Calendar meeting-response replay default or object constraint is missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Calendar meeting-response replay default and object constraint are current"

calendar_event_projection_state_shape_status="$(calendar_event_projection_state_shape_ok "$DATABASE_URL")" \
  || fail "Unable to inspect the Calendar Event projection-state default and constraint"
[[ "$calendar_event_projection_state_shape_status" == "1" ]] \
  || fail "Calendar Event projection-state default or constraint is missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Calendar Event projection-state default and constraint are current"

calendar_meeting_request_correlation_index_shape_status="$(calendar_meeting_request_correlation_index_shape_ok "$DATABASE_URL")" \
  || fail "Unable to inspect the Calendar meeting-request correlation index"
[[ "$calendar_meeting_request_correlation_index_shape_status" == "1" ]] \
  || fail "Calendar meeting-request correlation index is missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
pass "Calendar meeting-request correlation index is current"
...
[[ "$deleted_calendar_event_constraint_count" == "2" ]] \
  || fail "Deleted Calendar object-kind constraints are missing or invalid. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_identity_constraint_count" == "3" ]] \
  || fail "MAPI identity key constraints do not require 22-byte SourceKey/InstanceKey and 17-24-byte ChangeKey XIDs. LPE ${expected_release_version} requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_calendar_event_move_change_key_constraint_count" == "2" ]] \
  || fail "Calendar Event move ChangeKey constraints do not accept 17-24-byte XIDs. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
