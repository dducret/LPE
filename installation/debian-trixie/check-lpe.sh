...
[[ "$mapi_custom_public_folder_item_constraint_count" -ge "1" ]] \
  || fail "MAPI custom property object-kind constraint does not allow public_folder_item. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
mapi_associated_config_shape_constraint_ok="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT COUNT(*) FROM pg_constraint WHERE conrelid = 'public.mail_change_log'::regclass AND contype = 'c' AND pg_get_constraintdef(oid) LIKE '%associated_config%';")" \
...
[[ "$mapi_associated_config_shape_constraint_ok" -ge "1" ]] \
  || fail "MAPI associated configuration replay shape constraint is stale. Initialize a fresh LPE ${expected_release_version} database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
