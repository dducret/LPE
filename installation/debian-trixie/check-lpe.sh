...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_named_properties');" | grep -qx 'mapi_named_properties' \
  || fail "Table public.mapi_named_properties is missing. LPE 0.5.1 requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_custom_property_values');" | grep -qx 'mapi_custom_property_values' \
  || fail "Table public.mapi_custom_property_values is missing. LPE 0.5.1 requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_custom_public_folder_item_constraint_count" -ge "1" ]] \
  || fail "MAPI custom property object-kind constraint does not allow public_folder_item. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_profile_settings');" | grep -qx 'mapi_profile_settings' \
  || fail "Table public.mapi_profile_settings is missing. LPE 0.5.1 requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_folder_profile_property_values');" | grep -qx 'mapi_folder_profile_property_values' \
  || fail "Table public.mapi_folder_profile_property_values is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "${mapi_special_folder_alias_shape_ok}" == "1" ]] \
  || fail "MAPI special-folder alias schema is missing or invalid. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_local_replica_range_shape_status" == "1" ]] \
  || fail "MAPI local replica range table shape is missing or invalid. Run /opt/lpe/src/installation/debian-trixie/update-lpe.sh for an existing 0.5.0-sql-v1 database, or initialize a fresh LPE 0.5.1 database with init-schema.sh."
...
[[ "$mapi_outlook_cache_fidelity_shape_status" == "1" ]] \
  || fail "MAPI WLink/configuration FAI fidelity shape is missing or invalid. Run /opt/lpe/src/installation/debian-trixie/update-lpe.sh for an existing 0.5.0-sql-v1 database, or initialize a fresh LPE 0.5.1 database with init-schema.sh."
...
[[ "$mapi_shortcut_group_column_count" == "2" ]] \
  || fail "MAPI navigation shortcut group/header columns are missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_shortcut_target_nullable" == "YES" ]] \
  || fail "MAPI navigation shortcut target_folder_id is still NOT NULL. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_shortcut_save_stamp_column_count" == "1" && "$mapi_shortcut_save_stamp_constraint_count" == "1" ]] \
  || fail "MAPI navigation shortcut save_stamp column is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_associated_config_messages');" | grep -qx 'mapi_associated_config_messages' \
  || fail "Table public.mapi_associated_config_messages is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_associated_config_shape_constraint_ok" -ge "1" ]] \
  || fail "MAPI associated configuration replay shape constraint is stale. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_low_dynamic_property_count" == "0" ]] \
  || fail "MAPI low dynamic named-property ids remain. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.search_folders_user_saved_name_idx');" | grep -qx 'search_folders_user_saved_name_idx' \
  || fail "User-saved Search Folder uniqueness index is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recipient_suggestions');" | grep -qx 'recipient_suggestions' \
  || fail "Table public.recipient_suggestions is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recipient_suggestions_active_email_idx');" | grep -qx 'recipient_suggestions_active_email_idx' \
  || fail "Recipient suggestions active-email uniqueness index is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recipient_suggestions_rank_idx');" | grep -qx 'recipient_suggestions_rank_idx' \
  || fail "Recipient suggestions ranking index is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.recoverable_items');" | grep -qx 'recoverable_items' \
  || fail "Table public.recoverable_items is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_account_column_count" == "3" ]] \
  || fail "Recoverable-item account retention/hold columns are missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_mailbox_column_count" == "1" ]] \
  || fail "Recoverable-item mailbox retention override column is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$managed_retention_mailbox_column_count" == "1" ]] \
  || fail "Managed retention folder mailbox tag column is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$managed_retention_mailbox_fk_count" == "1" ]] \
  || fail "Managed retention folder mailbox tag constraint is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_change_constraint_count" -ge "4" ]] \
  || fail "Recoverable-item change-log constraints are missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$recoverable_shape_constraint_ok" -ge "1" ]] \
  || fail "Recoverable-item replay shape constraint is stale. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$public_folder_table_count" == "6" ]] \
  || fail "Public-folder canonical tables are missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$public_folder_change_constraint_count" -ge "4" ]] \
  || fail "Public-folder change-log constraints are missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$public_folder_sync_constraint_count" == "2" ]] \
  || fail "Public-folder sync constraints are missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_identity_version_column_count" == "2" ]] \
  || fail "MAPI identity version column shapes are invalid; expected bigint/bytea NOT NULL. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$calendar_event_lifecycle_column_count" == "2" ]] \
  || fail "Calendar Event Deleted Items lifecycle columns are missing or invalid. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -tAc "SELECT to_regclass('public.mapi_calendar_event_identity_moves');" | grep -qx 'mapi_calendar_event_identity_moves' \
  || fail "Calendar Event identity-move table is missing. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$deleted_calendar_event_constraint_count" == "2" ]] \
  || fail "Deleted Calendar object-kind constraints are missing or invalid. Initialize a fresh LPE 0.5.1 database with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
...
[[ "$mapi_identity_constraint_count" == "3" ]] \
  || fail "MAPI identity key constraints do not require 22-byte SourceKey/InstanceKey and 17-24-byte ChangeKey XIDs. LPE 0.5.1 requires an empty database initialized with /opt/lpe/src/installation/debian-trixie/init-schema.sh."
