...
mapi_calendar_event_identity_retirement_shape_ok() {
  ...
  psql "${database_url}" -X -v ON_ERROR_STOP=1 -At <<'SQL'
SELECT CASE WHEN
  to_regclass('public.mapi_calendar_event_identity_retirements') IS NOT NULL
  AND (
    SELECT COUNT(*)
    FROM information_schema.columns
    WHERE table_schema = 'public'
  ...
  AND EXISTS (
    ...
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'p'
      AND pg_get_constraintdef(oid) = 'PRIMARY KEY (tenant_id, account_id, old_mapi_object_id)'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'u'
      AND pg_get_constraintdef(oid) = 'UNIQUE (old_mapi_object_id)'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'u'
      AND pg_get_constraintdef(oid) = 'UNIQUE (old_source_key)'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%old_mapi_object_id%'
      AND pg_get_constraintdef(oid) LIKE '%65535%' AND pg_get_constraintdef(oid) LIKE '%= 1%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%replacement_mapi_object_id%'
      AND pg_get_constraintdef(oid) LIKE '%65535%' AND pg_get_constraintdef(oid) LIKE '%= 1%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%octet_length(old_source_key) = 22%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%octet_length(replacement_source_key) = 22%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%retired_change_number >= 43%'
      AND replace(pg_get_constraintdef(oid), '''', '') LIKE '%retired_change_number < 140737454800896%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%old_mapi_object_id <> replacement_mapi_object_id%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%old_source_key <> replacement_source_key%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
  ...
  AND EXISTS (
    ...
    JOIN pg_namespace namespace_row ON namespace_row.oid = index_class.relnamespace
    JOIN pg_am access_method ON access_method.oid = index_class.relam
    WHERE index_row.indrelid = to_regclass('public.mapi_calendar_event_identity_retirements')
      AND namespace_row.nspname = 'public'
  ...
  AND to_regclass('public.mapi_object_identity_claims') IS NOT NULL
  AND (
    SELECT COUNT(*)
    FROM information_schema.columns
    WHERE table_schema = 'public'
      ...
      AND (
        (column_name = 'mapi_object_id' AND data_type = 'bigint' AND is_nullable = 'NO')
        OR (column_name = 'source_key' AND data_type = 'bytea' AND is_nullable = 'NO')
        OR (column_name = 'claimed_at' AND data_type = 'timestamp with time zone'
          AND is_nullable = 'NO' AND column_default IS NOT NULL)
      )
  ) = 3
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_object_identity_claims')
      AND contype = 'p'
      AND pg_get_constraintdef(oid) = 'PRIMARY KEY (mapi_object_id)'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = to_regclass('public.mapi_object_identity_claims')
  ...
  AND EXISTS (
    ...
    WHERE conrelid = to_regclass('public.mapi_object_identity_claims')
      AND contype = 'c'
      AND pg_get_constraintdef(oid) LIKE '%mapi_object_id%'
      AND pg_get_constraintdef(oid) LIKE '%65535%' AND pg_get_constraintdef(oid) LIKE '%= 1%'
  )
  AND EXISTS (
    SELECT 1 FROM pg_constraint
  ...
  AND EXISTS (
    ...
    WHERE trigger_row.tgrelid = to_regclass('public.mapi_object_identities')
  ...
  AND EXISTS (
    ...
    JOIN pg_proc function_row ON function_row.oid = trigger_row.tgfoid
    JOIN pg_namespace namespace_row ON namespace_row.oid = function_row.pronamespace
    WHERE trigger_row.tgrelid = to_regclass('public.mapi_special_folder_aliases')
  ...
  AND EXISTS (
    ...
    JOIN pg_proc function_row ON function_row.oid = trigger_row.tgfoid
    JOIN pg_namespace namespace_row ON namespace_row.oid = function_row.pronamespace
    WHERE trigger_row.tgrelid = to_regclass('public.mapi_object_identity_claims')
  ...
  AND NOT EXISTS (
    ...
    LEFT JOIN public.mapi_object_identity_claims claim
  ...
  AND NOT EXISTS (
    ...
    LEFT JOIN public.mapi_object_identity_claims old_claim
      ON old_claim.mapi_object_id = retirement.old_mapi_object_id
     AND old_claim.source_key = retirement.old_source_key
    LEFT JOIN public.mapi_object_identity_claims replacement_claim
...
canonical_schema_shape_is_current() {
  ...
  [[ "$(mapi_calendar_event_identity_retirement_shape_ok "${database_url}")" == "1" ]] || return 1
