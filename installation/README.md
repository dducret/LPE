...
- `update-lpe.sh` performs the schema-version check, applies documented idempotent SQL compatibility updates for initialized `0.4` databases, including the recoverable-items store and replay-shape constraints, public-folder replica topology table, MAPI navigation shortcut columns, MAPI associated configuration message rows, active MAPI source-key uniqueness, and user-saved Search Folder uniqueness index, rebuilds code/web assets, and refuses to continue when required baseline tables or schema metadata are missing
