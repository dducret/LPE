...
- The public `/rpc/rpcproxy.dll` route must use streaming proxy settings equivalent to `/mapi/`: long read/send timeouts, `proxy_buffering off`, and `proxy_request_buffering off`. Without those settings, RCA can open the IN channel but the core service sees `total_request_body_bytes=0` and the endpoint ping times out with `1818 CallCancelled`.
