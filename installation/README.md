...
New LPE `0.5.2` installations initialize an empty SQL database from the
canonical schema with `init-schema.sh`. There is no in-place schema upgrade
path to this baseline. `update-lpe.sh` accepts only a database whose installed
label exactly matches the canonical schema label, verifies the required
canonical physical shape, and rejects every other label or incomplete
same-label database before it stops LPE or changes the database. Use a new empty database and
`init-schema.sh` for a rebuilt deployment. `LPE_RESET_SCHEMA=true` is only for
an intentional destructive reset.

`init-schema.sh` refuses to run while `lpe.service` (or `SERVICE_NAME`) is
active, resets and applies the canonical `public` schema in one transaction,
then validates the installed schema before announcing success. The installer
scripts use `psql -X` for their contract queries. Historical `0.5.0` and
`0.5.1` update artifacts remain preserved for release history but are not an
upgrade path to `0.5.2`.
...
- `update-lpe.sh` accepts only the exact current `0.5.2-sql` schema label with its required canonical physical shape and rejects every other label or incomplete same-label database before stopping LPE or changing the database; it then rebuilds code and web assets and restarts the services
...
for capable Outlook clients only when `LPE_AUTOCONFIG_MAPI_ENABLED` and
`LPE_AUTOCONFIG_MAPI_INTEROP_GATE_PASSED` are enabled, a legacy top-level
`EXCH` provider only when `LPE_AUTOCONFIG_EXCH_AUTODISCOVER_ENABLED` and
`LPE_AUTOCONFIG_EXCH_INTEROP_GATE_PASSED` are enabled with an explicitly
published EWS or fully gated MAPI surface, and a
legacy top-level `EXPR` provider only when
`LPE_AUTOCONFIG_EXPR_AUTODISCOVER_ENABLED` is also enabled with
`LPE_AUTOCONFIG_RPC_PROXY_ENABLED`,
`LPE_AUTOCONFIG_OUTLOOK_INTEROP_GATE_PASSED`, and a real
`/rpc/rpcproxy.dll` Outlook Anywhere path. `OPTIONS
...
authenticated `RPC_IN_DATA` mail-store channels return an open streaming
response while the client-to-server upload stream is drained asynchronously for
RCA payload logging.
...
- `MAPI over HTTP` routes are the classic Outlook desktop Exchange-account path; new 0.5.2 installations set `LPE_AUTOCONFIG_MAPI_ENABLED=true` but keep `LPE_AUTOCONFIG_MAPI_INTEROP_GATE_PASSED=false`, so capable Outlook clients receive Autodiscover MAPI metadata only after the documented public-edge evidence is recorded. Per [MS-OXDSCLI] sections 2.2.2.1 and 3.2.5.1, `X-MapiHttpCapability` is negotiation rather than proof of usability. SOAP Exchange `GetUserSettings`, legacy `EXCH`, and legacy `EXPR` remain separately controlled; `EXPR` still requires the RPC proxy and legacy interoperability gate
...
- The public `/mapi/` and `/rpc/rpcproxy.dll` routes must use streaming proxy settings: long read/send timeouts, `proxy_buffering off`, `proxy_request_buffering off`, and `client_max_body_size 0`. For MAPI `NotificationWait`, this preserves the immediate `PROCESSING` and periodic `PENDING` frames required by MS-OXCMAPIHTTP §3.2.5.2. LPE also returns `X-Accel-Buffering: no` for `NotificationWait` as a defence-in-depth signal to nginx, but it does not replace the required route configuration. RCA `RPC_IN_DATA` opens a long upload channel with a very large advertised request body; without the explicit body-size override, nginx can reject the mailbox-store channel with `413` before the core service can drain it.
- Microsoft Remote Connectivity Analyzer Outlook Connectivity expects top-level `EXCH` and `EXPR` provider sections; the errors "The EXCH provider section is missing from the Autodiscover response" or "The EXPR Provider section is missing in the Autodiscover response" mean that provider metadata is not published. Do not turn on a gate merely to satisfy RCA discovery: set `LPE_AUTOCONFIG_MAPI_INTEROP_GATE_PASSED`, `LPE_AUTOCONFIG_EXCH_INTEROP_GATE_PASSED`, and the legacy `LPE_AUTOCONFIG_OUTLOOK_INTEROP_GATE_PASSED` only after their respective real transports have local tests, public readiness, Microsoft RCA, and real Outlook profile/sync/reconnect evidence. For the EWS compatibility path, set `LPE_AUTOCONFIG_EWS_ENABLED=true`, `LPE_AUTOCONFIG_EXCH_AUTODISCOVER_ENABLED=true`, `LPE_AUTOCONFIG_EXCH_INTEROP_GATE_PASSED=true`, `LPE_AUTOCONFIG_EXPR_AUTODISCOVER_ENABLED=true`, `LPE_AUTOCONFIG_RPC_PROXY_ENABLED=true`, and `LPE_AUTOCONFIG_OUTLOOK_INTEROP_GATE_PASSED=true` only after that evidence is recorded.
- no client `SMTP` endpoint should be advertised unless the authenticated `LPE-CT` submission listener is configured, exposed on `465`, covered by the public certificate, and `LPE_AUTOCONFIG_SMTP_SUBMISSION_ENABLED=true` is set with its public hostname. [MS-OXSMTP] sections 2.2.1 and 3.2.5.1 and [MS-XLOGIN] section 2.2 apply only to that listener; port 25 must not offer `AUTH`
- the internal `LPE -> LPE-CT` relay must never be advertised as a client-submission endpoint

The `LPE_PUBLIC_SCHEME`, `LPE_PUBLIC_HOSTNAME`, `LPE_AUTOCONFIG_IMAP_HOST`, `LPE_AUTOCONFIG_IMAP_PORT`, `LPE_AUTOCONFIG_SMTP_HOST`, `LPE_AUTOCONFIG_SMTP_SUBMISSION_ENABLED`, `LPE_AUTOCONFIG_SMTP_PORT`, `LPE_AUTOCONFIG_SMTP_SOCKET_TYPE`, `LPE_AUTOCONFIG_EWS_ENABLED`, `LPE_AUTOCONFIG_EWS_URL`, `LPE_AUTOCONFIG_MAPI_ENABLED`, `LPE_AUTOCONFIG_MAPI_INTEROP_GATE_PASSED`, `LPE_AUTOCONFIG_OUTLOOK_INTEROP_GATE_PASSED`, `LPE_AUTOCONFIG_EXCH_AUTODISCOVER_ENABLED`, `LPE_AUTOCONFIG_EXCH_INTEROP_GATE_PASSED`, `LPE_AUTOCONFIG_EXPR_AUTODISCOVER_ENABLED`, `LPE_AUTOCONFIG_RPC_PROXY_ENABLED`, `LPE_AUTOCONFIG_SOAP_EXCHANGE_AUTODISCOVER_ENABLED`, `LPE_AUTOCONFIG_MAPI_EMSMDB_URL`, `LPE_AUTOCONFIG_MAPI_NSPI_URL`, and `LPE_AUTOCONFIG_WEBMAIL_URL` variables let you align the published HTTP/XML settings with the real public hostname. The detailed behavior is documented in `docs/architecture/client-autoconfiguration.md`.
...
If RCA reports only a Microsoft "Correlation ID", use the test timestamp and
the nearby `rca_debug=true` entries to identify the last successful or failing
Autodiscover, EWS, or MAPI request. The Microsoft correlation ID is generated by
RCA and is not guaranteed to be sent to the server.

Before starting an RCA Outlook Connectivity run, the same public publication can
be checked from the repository with:

```bash
LPE_RCA_PASSWORD='mailbox-password' \
  tools/rca_outlook_connectivity_check.py \
  --base-url https://l-p-e.ch \
  --email test@l-p-e.ch \
  --expected-service-host mail.l-p-e.ch \
  --expect-ews \
...
`update-lpe.sh` first checks that the installed schema label is exactly the
...
while `lpe.service` (or `SERVICE_NAME`) is active. This reset is destructive.
...
When invoking Bash from a Windows or PowerShell shell, pass the command through
`bash -c`, for example:
