...
- `update-lpe.sh` performs the schema-version check, applies documented idempotent SQL compatibility updates for initialized `0.4` databases, including the recoverable-items store, public-folder replica topology table, and MAPI navigation shortcut columns, rebuilds code/web assets, and refuses to continue when required baseline tables or schema metadata are missing
