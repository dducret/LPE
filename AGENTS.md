...
- for MAPI over HTTP Outlook view compatibility, do not advertise synthetic folder-local `IPM.Microsoft.FolderDesign.NamedView` rows in folder-associated tables, advertise Inbox mail default view EntryIDs through the visible Common Views `Compact` named-view row, expose modeled broad-startup Inbox `IPM.Configuration.*` rows needed by Outlook before Inbox content handoff, do not replay `IPM.ExtendedRule.Message` associated rows during startup enumeration, keep Sync Issues as a leaf in startup hierarchy tables until its child folders are backed by real behavior, and keep Quick Step Settings out of IPM subtree startup hierarchy enumeration until it is backed by real behavior; keep Common Views projections and persisted Outlook-created associated configuration rows separate, and only widen folder-local view exposure after real Outlook traces and `[MS-OXOCFG]` tests prove the complete descriptor contract
