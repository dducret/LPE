...
- MAPI over HTTP password or app-password verification occurs when an authenticated EMSMDB `Connect` or NSPI `Bind` actually establishes or reconnects a Session Context, and that boundary emits one `mail-auth.mapi.login-succeeded` audit event. Later requests must authenticate through the transport-, endpoint-, and account-bound `MapiContext`, a process-keyed credential proof, and a current active credential-verifier check; they must not repeat Argon2 verification or success-audit writes, must not retain plaintext credentials or password hashes in session state, and must still audit rejected credentials or changed authentication context. Direct MAPI/HTTP Session Contexts and legacy RPC context handles are not interchangeable
...
- for inbound Meeting Responses, accept only the exact base media type `text/calendar`, persist authorization only as the exact selected scheduling-part SHA-256 after ATTENDEE, SMTP MAIL FROM, unambiguous RFC From, and any supplied ORGANIZER match the recipient account, and recheck the complete selected-part and authorization fingerprint under the message lock before lazy repair; keep the client-only `PidTagProcessed` absent, and project canonical applied, idempotent, or superseded unique Event correlation as `PidLidServerProcessed=TRUE` with `cpsUpdatedCalItem` in `PidLidServerProcessingActions`, while unresolved or ambiguous responses expose neither processing marker; a `COUNTER` must carry its proposed duration, must never expose `PidLidIsSilent=TRUE`, and must project one complete persisted proposed flag/start/end tuple on the organizer Meeting's corresponding attendee row consistently through direct open/read/refresh and cached-mode sync. Keep the direct Calendar Event recipient-column contract invariant across open, reload, and later reads, and project the attendee's durable response time and the same EX/SMTP SearchKey identity in direct and cached recipient rows
- for MAPI over HTTP handle state, distinguish a never-assigned handle (`ecNullObject`) from an issued then released or closed handle (`ecInvalidObject`), keep server handle allocation independent of client-supplied numeric high-water values, and preserve a successfully saved Message long enough for required later ROPs in the same Execute buffer to observe its complete committed state, including recipient replacements
- for MAPI over HTTP ICS upload, follow `[MS-OXCFXICS]` sections 3.1.5.3 and 3.1.5.6: a new or non-conflicting Calendar Event Save preserves the imported MID/SourceKey/ChangeKey/PCL/LMT while allocating a distinct server-internal CN; a client-winning conflict keeps the imported ChangeKey and LMT with the merged predecessor lineage, while a server-winning conflict keeps the server ChangeKey and LMT with that merged lineage. Validate and canonicalize the Global/Clean Global Object ID pair before correlation, including native and third-party `vCal-Uid` forms; reject a malformed or mismatched pair and fail closed on a top-level exception/occurrence until canonical standalone-exception import exists. When LPE's GOID correlation policy resolves a different valid client-reserved MID/SourceKey to the same canonical active Event, rekey only that principal's active MAPI projection, return the new MID from Save, durably retire the old MID/SourceKey pair, and replay the exact old MID as a deletion; retain the canonical Event UUID and apply the same content-winner rules. A global durable identity-claim ledger must prevent any MAPI object or client-reserved special-folder alias from reusing either member of any previously assigned internal-ID/SourceKey pair; reject startup when live identities, aliases, or either side of retirement history lack their exact ledger claim, and block UPDATE, DELETE, and TRUNCATE of the ledger. Only content actually reflected in the uploading replica enters its upload `CnsetSeen`; a retained server winner must remain download-eligible. Every snapshot-backed Calendar contents row, including a custom Calendar, must use the same durable CN/ChangeKey/PCL/LMT tuple as opening that Event. GOID correlation is an LPE interoperability policy, not a Microsoft-mandated ICS identity rule. Only a later direct modification replaces a foreign ChangeKey with the XID for its new server CN, merges that XID into the predecessor lineage, and assigns a server LMT. Keep this durable version LMT separate from the canonical Event local commit time. An LPE interoperability capture is not Exchange-product evidence and must not override this imported-version rule
- for MAPI Calendar meeting state, project `PidLidResponseStatus` from the mailbox owner's actual role and canonical attendee participation: Appointment `0`, organizer `1`, and attendee tentative/accepted/declined/not-responded `2`/`3`/`4`/`5`. Importing attendee response status may update only that mailbox-owner attendee and must preserve every other attendee's response, RSVP, and counter-proposal state
- for canonical native-client submission, normalize adapter-specific trusted source labels to their protocol before queue persistence, compose an absent full MAPI subject as the exact `PidTagSubjectPrefix + PidTagNormalizedSubject` without double-prefixing an existing `PidTagSubject`, and resolve one safe `Date`/`Message-ID` before writing the Sent message, raw MIME, and outbound handoff so every canonical representation has the same identity
- treat `PidTagAdditionalRenEntryIds` as versioned Inbox hierarchy state: Root writes are aliases for that same Inbox property, documented indexes remain canonical server-owned EntryIDs, and opaque later indexes are preserved. Every accepted direct write or hierarchy import must merge the client patch under the durable Inbox lock and commit the normalized property value, validated special-folder aliases, Inbox CN/ChangeKey/PCL/LMT, and one hierarchy replay row atomically; hierarchy `folderChange` export must carry that exact committed `PtypMultipleBinary` value. Before export, lazily materialize or normalize legacy state and record its durable hierarchy-projection version with one Inbox rotation/journal so an existing OST's `CnsetSeen` cannot suppress the repair. Never acknowledge one version tuple while dropping its property bag, and never persist a corrected value without rotating and journaling the Inbox version
...
- for MAPI notifications, preserve the complete explicit `RopRegisterNotification` scope: a nonzero MessageId restricts delivery to that exact FolderId/MessageId pair, while a zero MessageId is folder scope; a committed provider-property mutation on `LocalFreebusy` writes its MAPI-only notification and Freebusy Data ICS replay row in the same transaction as the authoritative property bag and identity version, emits message `ObjectModified` to the saving and other live sessions, and replaces the saved handle from that complete bag, while a no-op Save does not. A same-folder Calendar identity replacement emits `ObjectDeleted` for the exact retired MID followed by `ObjectCreated` for the replacement MID in the saving and other live sessions; those distinct concrete MIDs must never be collapsed merely because both notifications carry the same canonical Event UUID, and only the replacement event drives the one parent hierarchy count refresh. For hierarchy tables, keep `NoNotifications (0x10)` and `SuppressesNotifications (0x80)` distinct: `0x10` disables the automatic table subscription, while `0x80` suppresses automatic table notifications caused by the current client's own action but remains eligible for later external changes; explicit notification registrations stay independent, and own-action correlation must remain Execute-local
- for actionable calendar mail, persist the exact first non-attachment iMIP scheduling MIME part: use the `[MS-OXCMAIL]` alternative/related selection rules and, for Outlook 2010 and later compatibility, search every `multipart/mixed` child in order as required by `[MS-STANOICAL]` V0334; later calendar candidates remain attachments. Persist its parser revision, durable classification generation, dirty-reclassification state, and bounded protocol metadata. Preserve the prior generation before replacing or deleting that MIME part; hydrate it through the active durable blob placement, revalidate its identity under the message lock, and reject future parser revisions. Track applied generation per visible account, locking ordered parent messages before account mail state, and rotate/journal every visible account exactly once for each actionable metadata transition, including actionable to `none`. Initial inbound and an account's first visible copied membership acknowledge the generation they were created with; this acknowledgement is insert-only so later mailbox copies or a re-copy over retained state cannot suppress a pending rotation. Expose metadata only from a single current/clean snapshot in which every visible account has applied the generation, retrying the repair/apply/read sequence on concurrent change. Normalize structurally valid native EncodedGlobalId and `mapi-goid:` UIDs to one internal `mapi-goid:<lowerhex>` form at every canonical Event write boundary while preserving malformed and ordinary UID case; accept an unprefixed native form only with the product-compatible uppercase ByteArrayID, zero reserved `X`, and an exact `Size`, and treat malformed private-prefixed values as third-party `vCal-Uid` data. Preserve occurrence-date bytes in `PidLidGlobalObjectId`, but zero them in `PidLidCleanGlobalObjectId` and when exporting a native EncodedGlobalId iCalendar UID; fail closed on outbound scheduling generation for a nonzero occurrence date until `RECURRENCE-ID` export is canonical. Before any outbound `METHOD:REQUEST` creates MIME, `Sent`, queue, or source-expunge state, require exactly one locked active, non-cancelled, non-recurring owner Event matching normalized UID, exact UTC interval, authorized organizer, unique canonical/request/envelope attendees, and a non-stale sequence; scheduling Bcc fails closed. Before outbound `REPLY` or `COUNTER` writes, require a completely parsed selected body whose attendee is the authorized `From`, exactly one visible organizer recipient, no Bcc, and any supplied ICS organizer equal to that recipient. Inbound responses must identify the attendee consistently in the ICS `ATTENDEE`, SMTP `MAIL FROM`, and RFC `From`; any supplied ICS `ORGANIZER` must match the receiving organizer mailbox. Retain a distinct RFC `Sender` as message metadata but do not use it as attendee authorization until delegate-response semantics are canonical. Fail closed unless UID plus any available sequence and original/scheduled interval evidence identifies exactly one active attendee-bearing Event across the owner's calendars; reject `RECURRENCE-ID` and decoded nonzero-occurrence-date responses until occurrence correlation is canonical
- for an actionable Meeting Request, keep request-side `PidTagProcessed` as per-account `mailbox_messages` state, separate from message-global response processing. Omit it while false; accept only the idempotent true transition on `SaveChangesMessage`, rotating and journaling the account's visible memberships only on the first transition. Preserve it across copy, move, and Recoverable Items restore, and reset it only when the actionable classification payload generation actually changes
