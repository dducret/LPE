...
- `EWS` is the active Exchange compatibility adapter; it must stay bounded to documented canonical mailbox, contacts, calendar, and submission behavior until its limits are explicitly widened
...
- for MAPI over HTTP Outlook view compatibility, do not advertise synthetic folder-local `IPM.Microsoft.FolderDesign.NamedView` rows in folder-associated tables, advertise Inbox mail default view EntryIDs through the visible Common Views `Compact` named-view row, expose only the modeled `IPM.Configuration.MessageListSettings` row in the Inbox associated table and for broad Inbox `IPM.Configuration.*` startup probes, do not replay other broad Inbox `IPM.Configuration.*` or `IPM.ExtendedRule.Message` associated rows during startup enumeration, keep Sync Issues as a leaf in startup hierarchy tables until its child folders are backed by real behavior, and keep Quick Step Settings out of IPM subtree startup hierarchy enumeration until it is backed by real behavior; keep Common Views projections and persisted Outlook-created associated configuration rows separate, and only widen folder-local view exposure after real Outlook traces and `[MS-OXOCFG]` tests prove the complete descriptor contract
