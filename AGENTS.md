...
- for inbound Meeting Responses, accept only the exact base media type `text/calendar`, persist authorization only as the exact selected scheduling-part SHA-256 after ATTENDEE, SMTP MAIL FROM, unambiguous RFC From, and any supplied ORGANIZER match the recipient account, and recheck the complete selected-part and authorization fingerprint under the message lock before lazy repair; clean reads take the processed bit from the canonical message, and `PidTagProcessed` may be true only for applied, idempotent, or superseded unique canonical Event correlation outcomes
