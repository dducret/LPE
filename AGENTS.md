...
- for MAPI over HTTP Outlook view compatibility, do not advertise synthetic folder-local `IPM.Microsoft.FolderDesign.NamedView` rows in folder-associated tables and do not replay broad Inbox `IPM.Configuration.*` or `IPM.ExtendedRule.Message` associated rows during startup enumeration; keep Common Views projections and persisted Outlook-created associated configuration rows separate, and only widen folder-local view exposure after real Outlook traces and `[MS-OXOCFG]` tests prove the complete descriptor contract
