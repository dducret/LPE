---
type: Rust Function
title: attachment_method_value_from_metadata
resource: crates/lpe-exchange/src/mapi/properties/attachments.rs#L71-L80
generated:
  by: okf-rs/0.3.0
relationships:
  calls:
  - functions/crates/lpe-exchange/src/mapi/properties/attachments/attachment_metadata_is_embedded_message
  called_by:
  - functions/crates/lpe-exchange/src/mapi/tables/attachments/serialize_pending_attachment_row
---

# Signature

`pub(in crate::mapi) fn attachment_method_value_from_metadata( media_type: &str, file_name: &str, ) -> u32`

# Calls

- [attachment_metadata_is_embedded_message](../../../../../../../functions/crates/lpe-exchange/src/mapi/properties/attachments/attachment_metadata_is_embedded_message.md)

# Called by

- [serialize_pending_attachment_row](../../../../../../../functions/crates/lpe-exchange/src/mapi/tables/attachments/serialize_pending_attachment_row.md)