...
| `ResolveNames` | Partial | Accounts, tenant directory rows, contact books/contacts and grants | Canonical address-book/contact lookup API | Resolves one visible canonical account or contact match; distribution lists are not projected. Invalid, unsupported-scope, distribution-list, and ambiguous lookups are parseable and expose no candidates. No full GAL templates or distribution-list expansion parity | P0 |
