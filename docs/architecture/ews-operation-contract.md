...
| `CopyItem` | Partial | `messages`, `mailbox_messages`, `public_folder_items`, change log/tombstones | Canonical message copy API; canonical public-folder item clone API | Supports at most 100 canonical message or public-folder item ids after target/source/access preflight; copy retains source membership and creates target membership | P1 |
| `DeleteItem` | Partial | `mailbox_messages`, contacts/events/tasks, `recoverable_items`, `public_folder_items`, change log/tombstones | Canonical delete, Trash move, collaboration delete, and public-folder item delete APIs | Calendar `MoveToDeletedItems` uses the canonical deleted-event lifecycle and `HardDelete` permanently deletes; contact deletion supports canonical `HardDelete` only. Unsupported contact/calendar delete modes are rejected before mutation; no full dumpster parity through EWS | P0 |
...
| `MoveItem` | Partial | `mailbox_messages`, target `mailboxes`, `public_folder_items`, change log/tombstones | Canonical message move API; canonical public-folder item clone/delete API | Supports at most 100 canonical message or public-folder item ids after target/source/access preflight; a mailbox move records target membership and source tombstone/change evidence | P1 |
...
| `UpdateItem` | Partial | Message flags/keywords, contacts, calendar, task rows, public-folder items, change log | Canonical update APIs for flags/read state, collaboration objects, and public-folder posts | Contact/calendar mutation requires a current EWS ChangeKey and validates the whole single-item mutation before writing. Multi-item contact/calendar batches are explicitly rejected until a canonical atomic batch transaction exists; mail updates are limited mainly to read/flag state | P0 |
...
| `EmptyFolder` | Partial | `mailbox_messages`, `recoverable_items`, `public_folder_items`, change log/tombstones | Canonical mailbox scoped delete API; canonical public-folder item delete API | Empties one bounded custom mailbox or accessible public-folder contents; protected/system folders, mixed targets, and undeletable descendants are rejected before mutation; optional subfolder deletion is limited to canonical deletable subfolders | P1 |
...
| `GetFolder` | Partial | Same as `FindFolder` | Canonical folder/collection fetch API | Returns exact supported mailbox/public-folder ids, root, supported distinguished mailbox roles, and accessible collaboration collections with bounded properties; inaccessible or unsupported distinguished ids return folder errors | P0 |
...
| `PerformReminderAction` | Partial | `reminder_occurrence_dismissals`, task/event reminder fields, message follow-up reminder dismissal state | Canonical reminder action API | Supports bounded dismiss and per-occurrence calendar/task snooze behavior; unsupported Exchange action shapes return EWS errors | P1 |
