...
| --- | --- | --- | --- | --- | --- |
| `GetDiscoverySearchConfiguration` | Partial | `discovery_searches` | Canonical compliance authorization is required before the compliance/admin search API can be exposed | Returns `ErrorAccessDenied`: an EWS mailbox principal has no canonical compliance authority, so no tenant search definitions are projected | P4 |
| `GetHoldOnMailboxes` | Partial | `compliance_holds`, `compliance_hold_mailboxes`, account litigation-hold fields | Canonical compliance authorization is required before the hold management API can be exposed | Returns `ErrorAccessDenied` before any canonical hold state is read | P4 |
| `GetNonIndexableItemDetails` | Partial | `non_indexable_item_reports` | Canonical compliance authorization is required before the search/index diagnostics API can be exposed | Returns `ErrorAccessDenied` before diagnostic or protected metadata can be projected | P4 |
| `GetNonIndexableItemStatistics` | Partial | `non_indexable_item_reports` | Canonical compliance authorization is required before the search/index diagnostics API can be exposed | Returns `ErrorAccessDenied` before diagnostic aggregates can be projected | P4 |
| `GetSearchableMailboxes` | Partial | Same-tenant `accounts` and litigation-hold fields | Canonical compliance authorization is required before the mailbox discovery API can be exposed | Returns `ErrorAccessDenied` before tenant mailbox enumeration | P4 |
| `SearchMailboxes` | Partial | `discovery_searches`, `discovery_search_jobs`, `discovery_result_items`, `mail_search_documents` | Canonical compliance authorization is required before the Bcc-safe compliance search API can be exposed | Returns `ErrorAccessDenied` before canonical search, result, or audit rows are written | P4 |
| `SetHoldOnMailboxes` | Partial | `compliance_holds`, `compliance_hold_mailboxes`, account litigation-hold fields, audit rows | Canonical compliance authorization is required before the hold mutation API can be exposed | Returns `ErrorAccessDenied` before any hold or audit mutation | P4 |
