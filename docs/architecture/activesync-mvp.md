...
- Protocol behavior:
  ...
  - rejects non-1.3, unterminated, overflowing, and trailing `WBXML` payload
    data before command dispatch, so malformed mobile input cannot create a
    SyncKey or reach canonical mailbox/contact/calendar mutation
    ([MS-ASWBXML] §2.1.2.1; [MS-ASCMD] §2.2.1)
