...
- unauthenticated `POST` commands return an HTTP `401` challenge instead of a generic `400`, so ActiveSync probes and clients can continue the normal HTTP authentication flow
- authenticated `OPTIONS` responses publish the `MS-Server-ActiveSync`, `MS-ASProtocolVersions`, and `MS-ASProtocolCommands` headers expected by ActiveSync client probes
