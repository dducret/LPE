...
| `MUST` | Internationalized mailbox names | IMAP mailbox-name parsing, validation, hierarchy matching, and rendering must use IMAP4rev2 UTF-8 behavior. `UTF8=ACCEPT` is part of the first internationalized mailbox release and may be advertised only when supported by command tests and real-client transcripts. |
...
| `MUST` | Mailbox name safety | JMAP mailbox create/update paths must use the strict Unicode mailbox-name policy in `docs/i18n.md`, including canonical-equivalence collision checks and mixed-script/confusable-name rejection. |
