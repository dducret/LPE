...
| `SHOULD` | `SPECIAL-USE` and folder role compatibility | Store standard backend names such as `INBOX`, `Sent`, and `Trash`; emit special-use attributes such as `\Archive`, `\Drafts`, and `\Junk` as role metadata for client presentation; keep compatibility aliases where real clients require them. |
