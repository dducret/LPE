...
| `MUST` | Internationalized mailbox names | IMAP mailbox-name parsing, validation, hierarchy matching, and rendering must use `docs/architecture/internationalized-mailbox-names.md` and IMAP4rev2 UTF-8 behavior. `UTF8=ACCEPT` is part of the first internationalized mailbox release and may be advertised only when supported by command tests and real-client transcripts. |
| `MUST` | Mailbox subscription state | `SUBSCRIBE`, `UNSUBSCRIBE`, and `LSUB` must use canonical persisted subscription state shared with JMAP `isSubscribed`; no IMAP-local subscription list is allowed. |
...
| `SHOULD` | `SPECIAL-USE` and folder role compatibility | Store standard backend names such as `INBOX`, `Sent`, and `Trash`; emit special-use attributes such as `\Archive`, `\Drafts`, and `\Junk` as role metadata for client presentation; keep compatibility aliases where real clients require them. Outlook/Exchange compatibility folders such as `Conversation History`, `RSS Feeds`, `Sync Issues`, and failure folders may be listed as ordinary mail folders when backed by canonical mailbox rows, but must not receive invented IMAP special-use flags. |
...
| `MUST` | Mailbox data | `Mailbox/get`, `Mailbox/query`, `Mailbox/changes`, and `Mailbox/set` where exposed must use canonical mailbox state and roles. Standard backend names remain stable; localized labels are client UI presentation driven by JMAP `role`. |
| `MUST` | Mailbox hierarchy and subscriptions | `Mailbox/*` must support `parentId` hierarchy for internationalized mailbox work, validate parent ownership and cycles, and expose `isSubscribed` from canonical persisted subscription state shared with IMAP `SUBSCRIBE`, `UNSUBSCRIBE`, and `LSUB`. |
| `MUST` | Mailbox name safety | JMAP mailbox create/update paths must use the strict Unicode mailbox-name policy in `docs/architecture/internationalized-mailbox-names.md`, including NFC display storage, canonical-key sibling collision checks, reserved-name protection, delimiter rejection, and mixed-script/confusable-name rejection. |
...
| --- | --- | --- | --- |
| Mailboxes and roles | Core LPE mailbox state | `LIST`, `XLIST`, `STATUS`, `SELECT`, `EXAMINE`; compatibility receive-folder and folder move/copy probes must map back to canonical mailbox roles rather than IMAP-local routing or hierarchy state. Exchange compatibility mail folders such as `Junk`, `Archive`, `Outbox`, `RSS Feeds`, `Conversation History`, and `Sync Issues` are ordinary mailboxes unless a standard IMAP special-use role exists. | `Mailbox/*`; compatibility receive-folder and folder move/copy probes must map back to canonical mailbox roles rather than JMAP-local routing or hierarchy state. JMAP exposes Exchange compatibility mail folders as mailbox names and roles only when they are backed by canonical mailbox rows; contacts, tasks, RSS object, FAI, free/busy, and search-folder semantics remain outside JMAP Mail. |
...
| Flags and keywords | Core LPE message state | system flags and keywords | `keywords`; Outlook follow-up metadata is exposed only through the private `xLpeFollowUp` `Email/get` property when explicitly requested, not as standard JMAP Mail state. The web client flag and completion controls write through the same canonical mailbox-message flag mutation path so JMAP, IMAP, ActiveSync, and MAPI projections observe the update. |
...
| Flags and read state | MAPI read/flag mutations update canonical mailbox-message flags, preserve the IMAP UID, and advance the mod-sequence. | `Email/get` keyword/read projections reflect the same canonical flags; private `xLpeFollowUp` reflects Outlook follow-up metadata without changing standard JMAP Mail semantics. | `RopSetReadFlags`, ICS read-state import, JMAP private follow-up projection, and storage runtime flag-mutation checks. |
