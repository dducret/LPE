...
- `Blob/get` returns requested byte ranges from readable temporary upload blobs or reconstructed canonical message blobs, including `digest:sha-256` over the returned octet range; non-owner delegated/shared mailbox reads use the same Bcc-safe reconstruction path as HTTP download
