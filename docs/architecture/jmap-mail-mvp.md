...
- `POST /api/jmap/api` enforces the `Session` core `maxSizeRequest` and `maxCallsInRequest` limits, and the core capability advertises `maxConcurrentRequests` plus object limits for real-client planning
