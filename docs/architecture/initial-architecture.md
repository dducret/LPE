...
- current `0.1.3` Exchange compatibility focus: `EWS` plus full classic Outlook `MAPI over HTTP`, implemented without moving `SMTP` or canonical mailbox state out of `LPE`
- `MAPI over HTTP` and Outlook Anywhere / RPC over HTTP are `0.1.3` release targets for classic Outlook for Windows Exchange-account support, with autodiscover publication gated until profile creation, EMSMDB, NSPI, RPC proxy channel establishment, session context, canonical mailbox synchronization, send, reconnect, and `Sent` behavior pass interoperability testing
- LPE code: `Apache-2.0`
...
`ActiveSync` is the first targeted mobile/native compatibility layer for clients that actually support `Exchange ActiveSync`, such as Outlook mobile and iOS mail clients. Outlook for Windows desktop must not be forced into `ActiveSync` as an Exchange account. `IMAP` was the development-start compatibility path through `0.1.2` and remains a supported mailbox-access communication protocol for desktop and other IMAP clients. Release `0.1.3` makes Exchange-style desktop compatibility a first-class release target through the `EWS` adapter, full classic Outlook `MAPI over HTTP`, and Outlook Anywhere / RPC over HTTP for legacy `EXCH` / `EXPR` RCA and profile flows. `mapiHttp` and legacy `EXCH` / `EXPR` autodiscover publication stay explicit until the Outlook interoperability matrix proves the implementation.
...
The `0.1.3` `EWS` adapter in `lpe-exchange` follows the same rule for mailbox, contacts, calendar, and task synchronization. `FindFolder`, `GetFolder`, `SyncFolderHierarchy`, `FindItem`, `GetItem`, `SyncFolderItems`, selected `CreateItem`, selected `DeleteItem`, `CreateFolder`, and `DeleteFolder` reuse canonical `LPE` storage and submission paths without introducing Exchange-specific storage or rights. It is not a complete Exchange server. `MAPI over HTTP` is also a `0.1.3` release target and must complete Outlook-critical EMSMDB, NSPI, session, synchronization, send, and reconnect behavior over canonical `LPE` state before supported `mapiHttp` publication. The supported scope is detailed in `docs/architecture/ews-mapi-mvp.md`.

Client auto-configuration must publish only real endpoints. In v1, `Thunderbird` and Outlook for Windows desktop may receive `IMAP` settings and must advertise `SMTP` submission only when an authenticated client-submission endpoint is explicitly exposed; the internal `LPE -> LPE-CT` relay must never be described as a client-submission service. Outlook autodiscover must not advertise `ActiveSync` as a desktop Exchange route. `EWS` may be advertised only when the administrator explicitly enables the bounded mailbox, contacts, calendar, and task EWS endpoint and accepts its MVP limits. `MAPI over HTTP` `mapiHttp` autodiscover publication must remain administrator-enabled and must become generally supported only after the `0.1.3` Outlook interoperability matrix passes; legacy `EXCH` provider publication additionally requires `LPE_AUTOCONFIG_EXCH_AUTODISCOVER_ENABLED`, and legacy `EXPR` provider publication additionally requires `LPE_AUTOCONFIG_EXPR_AUTODISCOVER_ENABLED` plus `LPE_AUTOCONFIG_RPC_PROXY_ENABLED` for a real `/rpc/rpcproxy.dll` Outlook Anywhere path.
...
- Outlook support as an explicit `0.1.3` goal: Outlook mobile through `ActiveSync`, Outlook for Windows desktop through the supported `IMAP` path when configured that way, `0.1.3` `EWS` for Exchange-style mail, contacts, calendar, and task compatibility, and full classic Outlook desktop Exchange-account support through `MAPI over HTTP`
...
The current delivery phase is to finish the existing protocol adapters and complete the `MAPI over HTTP` implementation required for full classic Outlook desktop support. `IMAP` brought the compatibility foundation through `0.1.2` and remains supported as a mailbox-access communication protocol; `0.1.3` focuses on the already-selected `EWS` adapter and the Outlook-critical MAPI/HTTP release gate.
...
- `MAPI over HTTP` and Outlook Anywhere / RPC over HTTP: complete initial routes, authentication, EMSMDB, NSPI, `/rpc/rpcproxy.dll` RTS virtual-connection establishment, DCE/RPC mailbox endpoint pings, session-context, mailbox synchronization, send, reconnect, and canonical `Sent` behavior until real Outlook desktop login and day-two use are viable in `0.1.3`
