...
- `tools/jmap_tester_big_three_check.pl` is the operator-run `JMAP::Tester` harness for Big Three request batching, result references, state/change/queryChanges coherence, Blob upload/download, and delegated/shared-account projection. It requires an external Perl installation with `JMAP::Tester` available and two real same-tenant mailbox credentials.
