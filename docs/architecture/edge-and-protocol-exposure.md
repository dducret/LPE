...
public `25` and validates trusted TLS, autodiscover IMAP/SMTP publication,
public `993`, submission `465`, and the IMAP login path needed by Outlook
desktop. If Outlook fails while `lpe.service` receives no new log entries, the
first logs to inspect are `journalctl -u lpe-ct.service` because the client has
not reached the core protocol adapter.
