...
- The 0.5.3 updater accepts only the current `0.5.3-sql` schema label with its
...
| --- | --- | --- |
| Outlook 0.5.3 acceptance | Re-run Calendar, Contacts, Tasks, offline reopen, send/receive, read state, notifications, and restart against a fresh `0.5.3-sql` database and clean Outlook profile. | Real Outlook trace, no crash/dialog, canonical state visible across protocols. |
...
| Installation | Keep fresh-database initialization deterministic and reject every non-`0.5.3-sql` label or incomplete same-label schema before update-time service stop or database mutation. | Shell syntax, isolated schema-guard test, `check-lpe.sh`. |
