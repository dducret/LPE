...
- The 0.5.3 updater accepts only the current `0.5.2-sql` schema label with its
  required canonical physical shape and rejects every other label or incomplete
  same-label database before service stop or database mutation.
...
| --- | --- | --- |
| Outlook 0.5.3 acceptance | Re-run Calendar, Contacts, Tasks, offline reopen, send/receive, read state, notifications, and restart against a fresh `0.5.2-sql` database and clean Outlook profile. | Real Outlook trace, no crash/dialog, canonical state visible across protocols. |
...
| Contact-storage module split | Before adding more behavior to `workspace.rs` or `mapi_contacts.rs`, split contact persistence into field normalization/projection, general contact CRUD, and MAPI identity/version persistence modules. Preserve each transaction and durable change-log boundary; keep `workspace.rs` as API-facing wiring. | Focused storage contact, MAPI contact, JMAP contact, and runtime-schema tests, then `cargo test -p lpe-storage`. |
...
| Installation | Keep fresh-database initialization deterministic and reject every non-`0.5.2-sql` label or incomplete same-label schema before update-time service stop or database mutation. | Shell syntax, isolated schema-guard test, `check-lpe.sh`. |
