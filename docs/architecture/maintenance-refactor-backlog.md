...
- The 0.5.2 updater accepts only the current `0.5.2-sql` schema label with its
...
| --- | --- | --- |
| Outlook 0.5.2 acceptance | Re-run Calendar, Contacts, Tasks, offline reopen, send/receive, read state, notifications, and restart against a fresh `0.5.2-sql` database and clean Outlook profile. | Real Outlook trace, no crash/dialog, canonical state visible across protocols. |
