...
Autodiscover publication is a compatibility contract, not only a routing hint. A route may exist internally before it is safe to publish. `EWS`, `mapiHttp`, legacy `EXCH`, and SOAP Exchange autodiscover blocks must remain separately gated so an administrator can expose the narrow surface they intend without hijacking the default Outlook desktop `IMAP` setup path.
...
- legacy Outlook and Remote Connectivity Analyzer probes without that header receive the top-level `EXCH` provider section only when `LPE_AUTOCONFIG_LEGACY_EXCHANGE_AUTODISCOVER_ENABLED` is also explicitly enabled; `EXPR` is not published because it advertises Outlook Anywhere / RPC over HTTP, which is not a supported `0.1.3` mailbox transport
...
`LPE_AUTOCONFIG_MAPI_ENABLED` must stay an explicit administrator switch until the classic Outlook for Windows desktop matrix proves profile creation, first sync, day-two sync, `NSPI` name resolution, canonical send and draft flows, reconnect behavior, and authoritative `Sent` visibility. Legacy `EXCH` publication has the stricter requirement that the deployment is intentionally testing Outlook setup probes that do not send `X-MapiHttpCapability`.
...
For Microsoft Remote Connectivity Analyzer Outlook Connectivity testing, an EWS-enabled deployment must also set `LPE_AUTOCONFIG_LEGACY_EXCHANGE_AUTODISCOVER_ENABLED=true`. RCA reports "The EXCH provider section is missing from the Autodiscover response" when only `LPE_AUTOCONFIG_EWS_ENABLED=true` is set, because the default EWS publication deliberately emits only the safer `WEB` EWS block. The legacy mode publishes a top-level `EXCH` provider section that carries the configured EWS URL for RCA validation. It deliberately does not publish top-level `EXPR`, because that tells RCA to validate Outlook Anywhere at `/rpc/rpcproxy.dll` and then to open a real RPC mailbox data channel. `LPE` exposes only a bounded authenticated compatibility shim for direct RPC proxy HTTP authentication probes, not a full RPC/HTTP mailbox data channel. Full Outlook desktop Exchange-account support remains the `MAPI over HTTP` release path, so RPC/HTTP is not mandatory for the `0.1.3` Outlook goal. Legacy mode must remain disabled on deployments where Outlook desktop should keep selecting the default `IMAP` profile.
...
- `LPE_AUTOCONFIG_LEGACY_EXCHANGE_AUTODISCOVER_ENABLED`, optional; set to `true`, `1`, `yes`, or `on` only for legacy Outlook / Remote Connectivity Analyzer `EXCH` provider testing after either `LPE_AUTOCONFIG_EWS_ENABLED` or `LPE_AUTOCONFIG_MAPI_ENABLED` is enabled, not for the default Outlook desktop `IMAP` path
...
- publish `mapiHttp` autodiscover only when `LPE_AUTOCONFIG_MAPI_ENABLED` is explicitly enabled for Outlook interoperability testing, and publish the legacy `EXCH` provider section only when `LPE_AUTOCONFIG_LEGACY_EXCHANGE_AUTODISCOVER_ENABLED` is also enabled with an explicitly published EWS or MAPI surface
