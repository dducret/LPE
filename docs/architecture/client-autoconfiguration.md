...
- legacy Outlook and Remote Connectivity Analyzer probes without that header receive the top-level `EXCH` and `EXPR` provider sections only when `LPE_AUTOCONFIG_LEGACY_EXCHANGE_AUTODISCOVER_ENABLED` is also explicitly enabled; publishing `EXPR` means the deployment is intentionally enabling the Outlook Anywhere / RPC over HTTP interoperability path
