...
- legacy Outlook and Remote Connectivity Analyzer probes without that header receive the top-level `EXCH` provider section only when `LPE_AUTOCONFIG_EXCH_AUTODISCOVER_ENABLED` is explicitly enabled, and receive the top-level `EXPR` provider section only when `LPE_AUTOCONFIG_EXPR_AUTODISCOVER_ENABLED` is explicitly enabled; publishing `EXPR` means the deployment is intentionally enabling the Outlook Anywhere / RPC over HTTP interoperability path
...
- `LPE_AUTOCONFIG_EXPR_AUTODISCOVER_ENABLED`, optional; set to `true`, `1`, `yes`, or `on` only for legacy Outlook / Remote Connectivity Analyzer top-level `EXPR` / Outlook Anywhere testing after either `LPE_AUTOCONFIG_EWS_ENABLED` or `LPE_AUTOCONFIG_MAPI_ENABLED` is enabled and `/rpc/rpcproxy.dll` is really published with authenticated RPC/HTTP behavior, not for the default Outlook desktop `IMAP` path
