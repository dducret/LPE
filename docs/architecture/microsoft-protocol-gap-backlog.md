...
| Priority | Protocol row(s) | Gap summary | Decision | Rationale | Required evidence | Proposed implementation owner | Tests/docs to update |
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | G110-G111 | Autodiscover publication and Outlook response-shape gaps. | bounded-implementation-with-retained-evidence | Endpoint publication is a gate, not feature breadth. [MS-OXDISCO] sections 2.2.5 and 3.2.3.3 require the public LPE-CT publication path; [MS-OXDSCLI] sections 2.2.2.1 and 3.2.5.1 make `X-MapiHttpCapability` negotiation, not proof of MAPI usability. `mapiHttp`, EXCH, and EXPR now retain separate enabled-transport and evidence gates. | Edge checks, scripted readiness, Microsoft RCA, and real Outlook profile evidence remain required before setting the MAPI, EXCH, or EXPR publication gate. | Autoconfiguration, LPE-CT edge, EWS/MAPI owners. | `docs/architecture/client-autoconfiguration.md`; edge install/test scripts; autodiscover tests. |
...
| P0 | G115 | SMTP AUTH behavior is bounded to LPE-CT authenticated submission; public ingress AUTH is intentionally unavailable. | bounded-implementation-with-retained-edge-evidence | SMTP belongs to LPE-CT. [MS-OXSMTP] sections 2.2.1 and 3.2.5.1 and [MS-XLOGIN] section 2.2 bound `AUTH PLAIN LOGIN` to the TLS submission listener; core LPE performs only signed canonical submission. Autodiscover requires its explicit authenticated-submission gate and never advertises the internal relay. | LPE-CT submission tests, public-ingress AUTH rejection, edge publication checks, and canonical Sent/submission trace evidence for each deployment. | LPE-CT SMTP/submission plus core submission owners. | `docs/architecture/client-autoconfiguration.md`; `docs/architecture/mail-security-and-traceability.md`; LPE-CT SMTP tests. |
...
| Priority | Protocol row(s) | Gap summary | Decision | Rationale | Required evidence | Proposed implementation owner | Tests/docs to update |
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | G029 | Spam/phishing metadata gaps. | bounded-implementation-with-retained-perimeter-evidence | Spam, phishing, reputation, quarantine, and transport security belong to LPE-CT. The sole safe core mailbox fact is the signed bridge-owned `x-lpe-ct-trace-id`; Internet-supplied values are stripped before handoff. [MS-OXCSPAM] section 2.2.1.3 SCL and [MS-OXPHISH] section 2.2.1.1 phishing stamps remain absent from canonical state. | Per-deployment LPE-CT filtering/trace evidence, quarantine operator isolation, and Bcc-safe user search/AI checks. | LPE-CT security and core mailbox projection owners. | `docs/architecture/mail-security-and-traceability.md`; `docs/architecture/lpe-ct-local-data-stores.md`; security tests; traceability docs. |
...
| P2 | G023-G024 | RTF compression and RTF encapsulation are not fully implemented. | needs-trace | Probe E `202608111835` proved that Outlook Sync Log Messages depend on compressed-RTF input when Body and HTML are absent. LPE now validates/decompresses bounded MS-OXRTFCP compressed and uncompressed containers and extracts canonical plain text, but does not preserve rich formatting, generate compressed output, or implement MS-OXRTFEX de-encapsulation. Cite MS-OXRTFCP sections 2.1.2-2.2.3 and MS-OXRTFEX 2.1-2.2. | Additional Outlook compose/read traces requiring rich formatting, objects, or HTML encapsulation; golden conversion fixtures for each adopted semantic. | MAPI body/property conversion owners. | MAPI body tests; conversion architecture notes for any widened rich-body scope. |
