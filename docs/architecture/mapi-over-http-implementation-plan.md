...
Inbox hierarchy `QueryRows`/`FindRow` rows, direct Inbox property reads, Root
fallback reads, and store logon reads must use the authenticated mailbox GUID
consistently so Outlook does not see distinct Calendar entry IDs for the same
default folder. Calendar content sync must
