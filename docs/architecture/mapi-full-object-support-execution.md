...
| [MS-OXCPRPT]: Getting Property IDs for Named Properties | https://learn.microsoft.com/en-us/openspecs/exchange_server_protocols/ms-oxcprpt/33c1b19f-0664-4b53-a968-2ee0d674f72b | Property operations use a 32-bit tag made from a 16-bit property ID and 16-bit type; named properties are mapped with `RopGetPropertyIdsFromNames` and use IDs with the high bit set. | Persistence scope for a non-Exchange implementation is an engineering decision; LPE must make mapping stable per mailbox/domain where Outlook caches require it. |
