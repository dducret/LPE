...
`RopQueryPosition` reports the current cursor and total row count for hierarchy, contents, and attachment tables. `RopResetTable` resets those cursors to the first row before subsequent `RopQueryRows` calls.
