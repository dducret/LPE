...
- autodiscover that publishes only the Exchange surfaces an administrator has explicitly enabled and that the interoperability matrix has proven, including legacy `EXCH` provider metadata for RCA only when its separate provider switch is enabled, and legacy `EXPR` provider metadata only when both its provider switch and the RPC proxy publication switch are enabled
...
- POP-before-SMTP or any direct client `SMTP` path inside the core `LPE` service
...
Full classic Outlook for Windows support in `0.1.3` is defined by `MAPI over HTTP` plus Outlook Anywhere / RPC over HTTP for legacy `EXCH` / `EXPR` RCA and profile flows. The mandatory release path is Outlook profile creation and day-two mailbox use through `/mapi/emsmdb`, `/mapi/nspi`, and `/rpc/rpcproxy.dll`, backed by canonical `LPE` mailbox, contacts, calendar, task, submission, and `Sent` state.

The `/rpc/rpcproxy.dll` route is mandatory for deployments that publish top-level `EXPR` or expose direct RCA Outlook Anywhere validation against the RPC proxy URL. It must satisfy the HTTP authentication probe and implement authenticated RPC/HTTP mailbox transport for the documented RCA and Outlook profile path.
...
`/rpc/rpcproxy.dll` exists for Outlook Anywhere setup probes and legacy `EXPR` provider flows in Microsoft Remote Connectivity Analyzer. All unauthenticated probes, including `RPC_IN_DATA` and `RPC_OUT_DATA`, reuse mailbox `MAPI` authentication and advertise `Basic` through a normal `401` challenge because anonymous is not a supported authentication method for this scenario. After authentication, `MS-RPCH` discovery probes sent with `RPC_IN_DATA`, `User-Agent: MSRPC`, or `Accept: application/rpc` return a 20-byte RTS ping echo with `Content-Type: application/rpc`. Authenticated `RPC_IN_DATA` mailbox-store, referral, and address-book endpoint channels return `200` headers immediately; for RCA's observed mailbox-store, referral, and NSPI address-book sequences, the first `:6001` IN channel uses an explicit zero-length response body to let the client proceed, and `:6002`, `:6004`, plus the following `:6001` IN channel are held open with a long RPC/HTTP response length and no initial response bytes while the upload stream is drained asynchronously. The drained IN channel now skips interleaved `MS-RPCH` RTS flow-control PDUs, parses complete DCE/RPC request fragments for the RCA bootstrap path, answers real DCE/RPC bind and alter-context PDUs with NTLM connect-level acknowledgements using the client's call ID, suppresses the next duplicate IN-channel bind acknowledgement when the matching OUT endpoint ping already supplied one, and can answer management `rpc_mgmt_inq_stats`, EMSMDB `EcDoConnectEx`, `EcDoRpcExt2`, and `EcDoDisconnect`, referral service `RfrGetNewDSA` and `RfrGetFQDNFromServerDN`, plus NSPI `Bind`, `Unbind`, `UpdateStat`, `QueryRows`, `SeekEntries`, `GetMatches`, `ResortRestriction`, `DNToMId`, `GetPropList`, `GetProps`, `CompareMIds`, `GetSpecialTable`, `GetTemplateInfo`, `QueryColumns`, `GetNamesFromIDs`, `GetIDsFromNames`, ASCII `ResolveNames`, and Unicode `ResolveNames` calls with minimal success PDUs. The RCA EMSMDB bootstrap now returns an authenticated session context handle and routes `EcDoRpcExt2` ROP payloads through the same EMSMDB executor used by `/mapi/emsmdb Execute`, including canonical mailbox snapshot loading. Bootstrap private-logon, folder, contents-table, and address-book rows are derived from the authenticated mailbox and canonical `LPE` store rather than from static RCA fallback values, without creating an Exchange-specific mailbox or GAL state model. The referral service returns the requested RPC endpoint host as the NSPI server FQDN for `:6002`, which keeps RCA's referral ping aligned with the published `:6004` address-book endpoint without introducing a separate directory-routing model. Authenticated `RPC_OUT_DATA` endpoint pings parse the client `CONN/A1` RTS PDU and answer with `CONN/A3` plus `CONN/C2`, deriving the receive window from the actual request. For RCA endpoint pings against the mailbox store (`:6001`), referral endpoint (`:6002`), and NSPI address book (`:6004`), the OUT channel also sends a minimal DCE/RPC bind acknowledgement after the RTS setup, including an NTLM connect-level challenge verifier, so RCA can complete the first RPC security leg instead of failing with an invalid-token access-denied error immediately after RTS setup. Both the normal OUT channel and the RCA endpoint OUT ping advertise a long RPC/HTTP content length and remain open after the initial response so the client can proceed to later DCE/RPC phases. Public reverse proxies for this route must disable request and response buffering, use long read/send timeouts, and disable the request-body size limit because Outlook Anywhere clients can advertise a 1 GiB `RPC_IN_DATA` upload channel. Without those settings, the edge can reject the IN channel with `413` or prevent the core service from observing the live upload stream, and RCA endpoint pings can still time out around the bind phase. The implementation path must continue from this RCA endpoint-ping compatibility behavior into full DCE/RPC bind parsing, endpoint ping, and mailbox/address-book proxying before top-level `EXPR` is treated as generally supported outside RCA-focused interoperability testing.
...
- NSPI `ResolveNames`: resolves same-tenant canonical account identities and accessible canonical contacts for the authenticated mailbox, reports the NSPI ANR marker `MID_RESOLVED` for a matched input name, honors the caller's requested address-book property columns when supplied, and returns no-match markers without static fallback rows for unresolved names
- NSPI table and property requests `GetProps`, `GetPropList`, `QueryColumns`, `QueryRows`, `GetSpecialTable`, `GetMatches`, `SeekEntries`, `UpdateStat`, `DNToMId`, `CompareMIds`, `GetTemplateInfo`, and `ResortRestriction`: return protocol-success data from a tenant-scoped projection of canonical `accounts` and accessible `contacts`, with stable entry ids and legacy DNs derived from canonical ids, without exposing a separate GAL store or cross-tenant directory data
- `PING`: returns a transport success response on either endpoint

`Execute` currently parses the EMSMDB Execute request body and validates the session cookie. It accepts both the legacy bare ROP payload used by the local compatibility tests and the `RPC_HEADER_EXT`-wrapped ROP payload used by Outlook/RCA, returning the same wrapped form when the request used it. It supports `RopRelease`, which intentionally emits no ROP response, the first private-mailbox `RopLogon` response with canonical account identity and fixed special-folder ids, `RopOpenFolder`, `RopCreateFolder`, `RopDeleteFolder`, `RopOpenMessage`, `RopCreateMessage`, `RopSetProperties`, `RopSetPropertiesNoReplicate`, `RopDeleteProperties`, `RopDeletePropertiesNoReplicate`, `RopModifyRecipients`, `RopRemoveAllRecipients`, `RopSaveChangesMessage`, `RopReloadCachedInformation`, `RopCreateAttachment`, `RopDeleteAttachment`, `RopSaveChangesAttachment`, `RopGetValidAttachments`, `RopOpenAttachment`, `RopOpenStream`, `RopReadStream`, `RopWriteStream`, `RopCommitStream`, `RopWriteAndCommitStream`, `RopReadRecipients`, `RopSetMessageReadFlag`, `RopSetReadFlags`, `RopGetMessageStatus`, `RopSetMessageStatus`, `RopSubmitMessage`, `RopMoveCopyMessages`, `RopDeleteMessages`, `RopHardDeleteMessages`, `RopGetHierarchyTable`, `RopGetContentsTable`, `RopGetAttachmentTable`, `RopSetColumns`, `RopSortTable`, `RopRestrict`, `RopQueryRows`, `RopFindRow`, `RopCreateBookmark`, `RopSeekRowBookmark`, `RopFreeBookmark`, `RopGetPropertiesSpecific`, `RopGetPropertiesAll`, `RopGetPropertiesList`, `RopGetNamesFromPropertyIds`, `RopGetPropertyIdsFromNames`, `RopQueryNamedProperties`, `RopGetStatus`, `RopQueryPosition`, `RopSeekRow`, `RopResetTable`, `RopGetReceiveFolder`, `RopGetReceiveFolderTable`, and `RopGetStoreState` for early mailbox-store bootstrap. The private-mailbox `RopLogon` response publishes only the response-valid `Private`, `Undercover`, and `Ghosted` logon flag bits even when Outlook sends additional request-only flags, uses proper Exchange Folder ID layout with a replica id plus non-zero global counters, and returns valid UTC `LogonTime` plus non-zero `GwartTime` fields. The implementation now loads a bounded `MapiMailStoreSnapshot` from canonical JMAP mailbox, message, and attachment projections before ROP execution, so attachment-table, body-stream, and mutation ROPs share one canonical mail-store boundary instead of re-querying or introducing MAPI-local state. A dedicated EMSMDB mail-store helper module owns deterministic source keys, change keys, predecessor change lists, local replica id allocation, and bootstrap synchronization manifests. These values are derived from canonical JMAP mailbox and message state, and message change tracking intentionally excludes protected `Bcc` recipients.

It keeps per-session ROP handle state and populates root and IPM-subtree hierarchy table rows from canonical JMAP mailbox folders, including display name, folder id, parent folder id, content count, unread count, subfolder flags, source key, parent source key, change key, predecessor change list, and change number for the supported property tags. `RopCreateFolder` creates canonical custom JMAP mailboxes and opens the resulting folder handle; `RopDeleteFolder` routes custom mailbox removal through canonical JMAP mailbox deletion and rejects system folders instead of allowing protocol-local hierarchy mutation. Contents tables expose read-only canonical message rows for a folder, including common subject, sender, recipient display, size, read/flag state, attachment, message id, entry id, instance-key, source key, parent source key, change key, predecessor change list, and change number columns. Hierarchy, contents, and attachment table handles track `RopQueryRows` cursor position and honor the requested row count for repeated table reads. `RopOpenMessage` validates the requested message id against canonical mailbox data and creates a message handle; `RopGetPropertiesSpecific` can return the same common message properties plus plain text body from that handle. `RopGetPropertiesAll` returns the default canonical property projection for folder, message, and attachment handles. `RopSetProperties` and `RopSetPropertiesNoReplicate` on opened canonical messages are intentionally limited to `PidTagMessageFlags` read-state changes and `PidTagFlagStatus` flag changes, routed through canonical JMAP message flags. Unsupported canonical message property writes fail instead of creating a MAPI-only extended property shadow. `RopCreateMessage` creates a transient per-session message handle, `RopSetProperties` and `RopSetPropertiesNoReplicate` store supported tagged properties on pending message and pending attachment handles and make them immediately readable from the same handle, `RopDeleteProperties` and `RopDeletePropertiesNoReplicate` remove those transient properties without mutating durable canonical messages, `RopModifyRecipients` adds, replaces, or removes pending recipients using the recipient row id and the supported SMTP/display-name/recipient-type columns, `RopRemoveAllRecipients` clears pending recipients after consuming the documented four-byte reserved request field, and `RopSaveChangesMessage` imports the pending message into the target canonical JMAP mailbox through the shared mailbox store. This save path currently supports the common text properties needed by the bootstrap slice, including subject, plain text body, sender display/address fallback, internet message id, and `To`/`Cc`/`Bcc` recipient vectors.

`RopSubmitMessage` sends unsaved pending messages and opened canonical `Drafts` messages through the same canonical submission path used by EWS and JMAP, preserving authoritative `Sent` and outbound queue behavior without creating protocol-local `Outbox` state. `RopCreateAttachment` creates a transient attachment handle on an opened canonical message, `RopSetProperties` and `RopSetPropertiesNoReplicate` can populate attachment filename, MIME type, and binary content, `RopDeleteProperties` and `RopDeletePropertiesNoReplicate` can clear transient pending-attachment properties and stream bytes before save, `RopOpenStream` plus `RopWriteStream` / `RopWriteAndCommitStream` can populate `PidTagAttachDataBinary`, and `RopSaveChangesAttachment` validates the bytes with Magika before routing the file through canonical message-attachment ingestion. `RopGetValidAttachments` enumerates valid canonical attachment numbers for an opened message, and `RopDeleteAttachment` removes existing canonical file attachments through the shared attachment deletion path. Named-property ROPs are implemented as a bounded session-local bootstrap map for Outlook property negotiation: `PS_MAPI` LID requests return their standard property IDs, `PS_INTERNET_HEADERS` string names are normalized to lowercase, new non-standard names are assigned IDs above `0x8000`, and the mapping is not yet a durable canonical extended-property store. MAPI submit currently submits the text and recipient surface already available on the message handle; outbound attachment submission will require the canonical draft/submission attachment model to be widened. `RopReloadCachedInformation` returns the same cached subject and recipient-count summary for opened canonical messages and unsaved pending messages without mutating canonical state. `RopReadRecipients` returns read-only recipient rows for canonical visible `To` and `Cc` recipients, includes protected `Bcc` recipients only when the opened message is in canonical `Drafts` or `Sent`, and can read the pending recipients from an unsaved `RopCreateMessage` handle. `RopGetAttachmentTable` creates a read-only attachment table over the opened message and `RopQueryRows` can return canonical file attachment number, filename, MIME type, size, method, rendering position, entry id, and instance key columns. `RopOpenAttachment` validates an attachment number from the opened message and creates a read-only attachment handle whose property reads use the same canonical attachment projection. `RopOpenStream` and `RopReadStream` support read-only `PidTagAttachDataBinary` streams backed by canonical attachment blob content, and writable streams on pending attachment handles before canonical save. `RopSetMessageReadFlag` and `RopSetReadFlags` update canonical JMAP message unread state through the shared mailbox store path; `RopSetReadFlags` reports partial completion when a requested message cannot be updated. `RopGetMessageStatus` and `RopSetMessageStatus` maintain bounded per-session message status flags for Outlook bootstrap compatibility while leaving durable canonical read, flag, and lifecycle state in the shared mailbox store. `RopMoveCopyMessages`, `RopDeleteMessages`, and `RopHardDeleteMessages` call the canonical JMAP mailbox mutation paths for message move, copy, trash, and hard delete behavior; soft delete moves to canonical `Trash` when present and falls back to hard delete only when no trash mailbox exists.

The first synchronization groundwork supports `RopSynchronizationConfigure`, `RopFastTransferSourceGetBuffer`, `RopSynchronizationOpenCollector`, `RopSynchronizationUploadStateStreamBegin`, `RopSynchronizationUploadStateStreamContinue`, `RopSynchronizationUploadStateStreamEnd`, `RopSynchronizationGetTransferState`, `RopSynchronizationImportHierarchyChange`, `RopSynchronizationImportMessageChange`, `RopSynchronizationImportDeletes`, `RopSynchronizationImportMessageMove`, `RopSynchronizationImportReadStateChanges`, `RopSetLocalReplicaMidsetDeleted`, and `RopGetLocalReplicaIds` as authenticated mailbox-store bootstrap primitives. Download sync exposes deterministic canonical manifest/state data and replica-scoped ids; upload sync can persist a collector state stream for the session, return that state as a fast-transfer source, map imported read-state and flag changes to canonical JMAP flags, route imported deletes and moves through canonical JMAP delete/move paths, create canonical custom mailboxes from uploaded hierarchy changes, reject uploaded system-folder mutations, and open a pending-message handle for imported new-message changes. Unsupported imported property writes still fail rather than creating MAPI-only state. This is not yet a complete Incremental Change Synchronization implementation with durable tombstone replay, conflict resolution, hierarchy move/rename support, or full fast-transfer object serialization. The common special-folder ids for `Inbox`, `Drafts`, `Sent`, and `Deleted` are also mapped back to canonical mailbox roles for opened-folder property reads, contents-table reads, message opens, receive-folder discovery, deletion targeting, and pending-message save targets. Other ROPs currently return MAPI ROP error buffers with `MAPI_E_NO_SUPPORT`. NSPI address-book responses now project tenant-scoped canonical accounts and accessible contacts, but they do not yet implement full NSPI restriction evaluation, distribution lists, or complete multi-recipient GAL search semantics.

`RopSortTable` stores per-table sort orders for hierarchy, contents, and attachment tables, resets the cursor, and applies supported canonical property comparisons before later `RopQueryRows` calls. `RopRestrict` stores a parsed table restriction for supported `AND`, `OR`, `NOT`, content, property, bitmask, size, and existence predicates over the canonical property subset; unsupported restriction shapes return `MAPI_E_NO_SUPPORT` instead of applying an implicit broad match. `RopQueryRows` honors table restrictions, requested row count, forward or backward read direction, and `NoAdvance` flag. `RopFindRow` evaluates an operation-local restriction against the currently restricted and sorted table view and returns row data using the active columns. `RopCreateBookmark`, `RopSeekRowBookmark`, and `RopFreeBookmark` maintain per-table cursor bookmarks for hierarchy and contents tables; bookmarks are invalidated when the table is reset, sorted, or restricted. `RopQueryPosition` reports the current cursor and total row count for hierarchy, contents, and attachment tables. `RopSeekRow` moves those table cursors from beginning, current, or end bookmarks and reports bounded row movement when requested. `RopResetTable` resets those cursors to the first row before subsequent `RopQueryRows` calls.
...
- message creation through canonical draft, submission, and mailbox import models
...
- add synchronization ROPs over canonical mailbox data
- deeper NSPI restriction, distribution-list, and multi-recipient search behavior without introducing a parallel GAL store
