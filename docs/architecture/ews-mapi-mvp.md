...
`/rpc/rpcproxy.dll` exists for Outlook Anywhere setup probes and legacy `EXPR` provider flows in Microsoft Remote Connectivity Analyzer. All unauthenticated probes, including `RPC_IN_DATA` and `RPC_OUT_DATA`, reuse mailbox `MAPI` authentication and advertise `Basic` through a normal `401` challenge because anonymous is not a supported authentication method for this scenario. After authentication, `MS-RPCH` discovery probes sent with `RPC_IN_DATA`, `User-Agent: MSRPC`, or `Accept: application/rpc` return a 20-byte RTS ping echo with `Content-Type: application/rpc`. Authenticated `RPC_IN_DATA` mailbox-store channels return headers immediately, advertise a long RPC/HTTP content length, remain open for the RPC/HTTP timeout, and drain the incoming stream asynchronously so DCE/RPC bind payloads can be observed and later bridged to the OUT channel. Authenticated `RPC_OUT_DATA` mailbox-store pings parse the client `CONN/A1` RTS PDU and answer with `CONN/A3` plus `CONN/C2`, deriving the receive window from the actual request. The OUT channel advertises a long RPC/HTTP content length and remains open after the initial RTS response so the client can proceed to the DCE/RPC bind phase. The implementation path must continue from this RTS virtual-connection establishment into DCE/RPC bind, endpoint ping, and mailbox proxying before top-level `EXPR` is treated as generally supported outside RCA-focused interoperability testing.
