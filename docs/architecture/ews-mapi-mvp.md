...
| Profile settings | `mapi_profile_settings` stores only bounded account-scoped settings required for Outlook cached-mode profile reuse. The current persisted setting is the IPM subtree OST identity written during profile bootstrap; default-folder EntryID properties remain computed canonical projections and are not stored as protocol-local state. |
...
| folder rule and deferred-action ROPs | `RopGetRulesTable` is implemented as a bounded read-only table over canonical Sieve-backed mailbox rules. `RopModifyRules` remains unsupported for rule mutation, and `RopUpdateDeferredActionMessages` remains unsupported until deferred action messages have a canonical `LPE` model. Exchange rule condition/action blobs, provider-specific predicates, client-only rules, and delegate rule templates are not stored. |
...
| search-folder criteria ROPs | Canonical built-in and user-saved Search Folder definitions are stored in `search_folders`, exposed through private JMAP/client APIs, and projected into MAPI hierarchy state. `RopSetSearchCriteria` and `RopGetSearchCriteria` requests are still parsed but unsupported with ROP-specific protocol errors until Outlook binary criteria can be round-tripped into the canonical JSON restriction model. |
