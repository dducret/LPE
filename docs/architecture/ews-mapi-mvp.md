...
- `MAPI over HTTP` currently has authenticated transport, session-context wiring, a private-mailbox logon skeleton, read-only canonical mailbox-folder bootstrap ROPs, an initial read-only contents-table view over canonical message rows, and read-only message open/property bootstrap; it is not an Outlook-ready mailbox service and must be advertised only when `LPE_AUTOCONFIG_MAPI_ENABLED` is explicitly enabled for interoperability testing
...
- add recipient row expansion, attachment table bootstrap, body stream handling, and synchronization ROPs over canonical mailbox data
