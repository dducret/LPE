...
`/rpc/rpcproxy.dll` exists only because legacy `EXPR` / Outlook Anywhere setup probes in Microsoft Remote Connectivity Analyzer test HTTP authentication and RPC proxy discovery against that URL. For ordinary unauthenticated HTTP probes it reuses mailbox `MAPI` authentication and advertises `Basic` through a normal `401` challenge. For `MS-RPCH` echo discovery probes sent with `RPC_IN_DATA` or `RPC_OUT_DATA`, `User-Agent: MSRPC`, and a 0 to 16 byte request body, it returns the 20 byte RTS echo response with `Content-Type: application/rpc`. Authenticated compatibility probes still return a small success response instead of a web-server `405`. It does not publish or implement the Outlook Anywhere RPC data channel; real Outlook desktop mailbox access remains the `/mapi/emsmdb` and `/mapi/nspi` `MAPI over HTTP` path.
