...
| Reconnect and failure behavior | Stale or missing cookies return parseable MAPI errors; session cookies are bounded to the authenticated principal; EMSMDB `Connect` with a valid session cookie rotates the cookie while preserving open session handles; duplicate byte-identical EMSMDB `Execute` request IDs replay the cached protocol response without repeating canonical mutations; the same request ID with a different ROP payload fails instead of guessing | session expiry unit test, missing-cookie/auth-context tests, disconnect/unbind tests, RPC context tests, `mapi_over_http_connect_reestablishes_session_context_with_open_sync_handle`, `mapi_over_http_replayed_execute_request_id_does_not_resubmit_message` | Cross-process session replay is not durable; deployments must keep MAPI session affinity or accept reconnect through fresh `Connect` / `Logon` probes |
