...
`/rpc/rpcproxy.dll` exists only because legacy `EXPR` / Outlook Anywhere setup probes in Microsoft Remote Connectivity Analyzer test HTTP authentication and RPC proxy discovery against that URL. All unauthenticated probes, including `RPC_IN_DATA` and `RPC_OUT_DATA`, reuse mailbox `MAPI` authentication and advertise `Basic` through a normal `401` challenge because anonymous is not a supported authentication method for this scenario. After authentication, `MS-RPCH` ping probes sent with `RPC_IN_DATA` or `RPC_OUT_DATA`, `User-Agent: MSRPC`, or `Accept: application/rpc` return the 20 byte RTS echo response with `Content-Type: application/rpc`, including RCA's `RPC_OUT_DATA` mailbox-store ping body. Other authenticated compatibility probes return a small success response instead of a web-server `405`. It does not publish or implement the Outlook Anywhere RPC data channel; real Outlook desktop mailbox access remains the `/mapi/emsmdb` and `/mapi/nspi` `MAPI over HTTP` path.
