...
EXAMPLE_FILE="${EXAMPLE_FILE:-${SCRIPT_DIR}/lpe-ct.env.example}"
STRICT=false
APPEND_MISSING=false

usage() {
  cat <<'USAGE'
Usage: check-lpe-ct-env.sh [options]

Compares an LPE-CT environment file with lpe-ct.env.example and prints the
KEY=value lines that are missing from the deployed file.

Options:
  --env-file PATH       Environment file to inspect. Default: /etc/lpe-ct/lpe-ct.env
  --example-file PATH   Reference example file. Default: sibling lpe-ct.env.example
...
if [[ "${APPEND_MISSING}" == "true" ]]; then
  {
    echo
    echo "# Added from $(basename "${EXAMPLE_FILE}") by check-lpe-ct-env.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)"
    printf '%s\n' "${missing_lines[@]}"
  } >> "${ENV_FILE}"
