...
type AccountPanelMode = "details" | "import" | "export";
...
function App() {
  ...
  const [accountEditForm, setAccountEditForm] = React.useState({ display_name: "", quota_mb: "4096", status: "active", password: "" });
  ...
  function openAccountPanel(account: AccountRecord, mode: AccountPanelMode) {
    ...
    setAccountEditForm({ display_name: account.display_name, quota_mb: String(account.quota_mb), status: account.status, password: "" });
      ...
      {state ? <>
        ...
        {page === "domain" ? <section className="page-card">
          ...
          {selectedDomain ? <>
            ...
            {domainTab === "accounts" ? <div className="account-workbench"><div className="account-left"><form className="card form-stack" onSubmit={(e)=>{e.preventDefault(); const email = `${accountLocalPart}@${selectedDomain.name}`; void mutate("account","console/accounts","POST",{ email, display_name: accountDisplayName || accountLocalPart, quota_mb: Number(accountQuota), password: accountPassword },copy.saved,()=>{setAccountLocalPart(""); setAccountDisplayName(""); setAccountQuota("4096"); setAccountPassword("");});}}><h3>{copy.accounts}</h3><Field label={copy.localPart} value={accountLocalPart} onChange={setAccountLocalPart} placeholder="alice" /><Field label={copy.displayName} value={accountDisplayName} onChange={setAccountDisplayName} placeholder="Alice" /><Field label={copy.quota} type="number" value={accountQuota} onChange={setAccountQuota} /><Field label={copy.password} type="password" value={accountPassword} onChange={setAccountPassword} /><button className="primary-button" disabled={busy==="account"} type="submit">{copy.create}</button><button className="secondary-button" type="button" disabled={busy==="pst-run"} onClick={()=>void runPstJobs()}>{copy.runPstJobs}</button></form><article className="card"><h3>{copy.domainAccounts}</h3><div className="account-list">{domainAccounts.map((account)=><div className={selectedAccountId===account.id?"account-list-item is-active":"account-list-item"} key={account.id} role="button" tabIndex={0} onClick={()=>openAccountPanel(account,"details")} onDoubleClick={()=>openAccountPanel(account,"details")} onKeyDown={(event)=>{ if (event.key === "Enter") openAccountPanel(account,"details"); }}><span className="account-main"><strong>{account.display_name}</strong><span>{account.email}</span></span><span className="account-meta"><span>{account.used_mb}/{account.quota_mb} MB</span><span className="pill">{account.status}</span></span><span className="account-actions" onClick={(event)=>event.stopPropagation()}><button className="icon-button" type="button" title={copy.pstImport} onClick={()=>openAccountPanel(account,"import")}>⇩</button><button className="icon-button" type="button" title={copy.pstExport} onClick={()=>openAccountPanel(account,"export")}>⇧</button></span></div>)}</div></article></div><aside className="card side-panel">{selectedAccount ? <div className="form-stack"><div className="side-panel-header"><div><h3>{accountPanelMode === "details" ? copy.accountDetails : accountPanelMode === "import" ? copy.pstImport : copy.pstExport}</h3><p className="muted">{selectedAccount.email}</p></div><div className="account-actions"><button className={accountPanelMode==="details"?"icon-button is-active":"icon-button"} type="button" onClick={()=>openAccountPanel(selectedAccount,"details")}>✎</button><button className={accountPanelMode==="import"?"icon-button is-active":"icon-button"} type="button" onClick={()=>openAccountPanel(selectedAccount,"import")}>⇩</button><button className={accountPanelMode==="export"?"icon-button is-active":"icon-button"} type="button" onClick={()=>openAccountPanel(selectedAccount,"export")}>⇧</button></div></div>{accountPanelMode === "details" ? <form className="form-stack" onSubmit={(e)=>{e.preventDefault(); void mutate(`account-${selectedAccount.id}`,`console/accounts/${selectedAccount.id}`,"PUT",{ display_name: accountEditForm.display_name, quota_mb: Number(accountEditForm.quota_mb), status: accountEditForm.status, password: accountEditForm.password || null },copy.saved,()=>setAccountEditForm((current)=>({...current,password:""})));}}><Field label={copy.displayName} value={accountEditForm.display_name} onChange={(value)=>setAccountEditForm((current)=>({...current,display_name:value}))} /><Field label={copy.quota} type="number" value={accountEditForm.quota_mb} onChange={(value)=>setAccountEditForm((current)=>({...current,quota_mb:value}))} /><label className="field"><span>{copy.status}</span><select value={accountEditForm.status} onChange={(event)=>setAccountEditForm((current)=>({...current,status:event.target.value}))}><option value="active">active</option><option value="disabled">disabled</option><option value="suspended">suspended</option></select></label><Field label={copy.newPassword} type="password" value={accountEditForm.password} onChange={(value)=>setAccountEditForm((current)=>({...current,password:value}))} placeholder="Optional" /><button className="primary-button" type="submit" disabled={busy===`account-${selectedAccount.id}`}>{copy.save}</button></form> : null}{accountPanelMode !== "details" ? <form className="form-stack" onSubmit={(e)=>{e.preventDefault(); if (!selectedMailbox) { setError(copy.noData); return; } const pstForm = pstFormFor(selectedMailbox.id); if (accountPanelMode === "import") { void uploadPstImport(selectedMailbox, pstForm.requested_by); return; } if (!accountTransferPath.trim()) { setError(copy.pstPath); return; } void mutate(`pst-${selectedMailbox.id}`,"console/mailboxes/pst-jobs","POST",{ mailbox_id: selectedMailbox.id, direction: accountPanelMode, server_path: accountTransferPath, requested_by: pstForm.requested_by },copy.saved,()=>setAccountTransferPath(""));}}>{selectedMailbox ? <p className="feedback muted">{copy.primaryMailbox}: {selectedMailbox.display_name}</p> : <p className="feedback error">{copy.noData}</p>}<Field label={copy.requestedBy} value={selectedMailbox ? pstFormFor(selectedMailbox.id).requested_by : (adminIdentity?.email ?? "admin@example.test")} onChange={(value)=>selectedMailbox ? setPstForms((current)=>({ ...current, [selectedMailbox.id]: { ...pstFormFor(selectedMailbox.id), requested_by: value, direction: accountPanelMode } })) : undefined} />{accountPanelMode === "import" ? <label className="field"><span>{copy.pstFile}</span><input type="file" accept=".pst" onChange={(event)=>setAccountImportFile(event.target.files?.[0] ?? null)} /></label> : <Field label={copy.pstPath} value={accountTransferPath} onChange={setAccountTransferPath} placeholder="/var/lib/lpe/exports/account.pst" />}<button className="primary-button" disabled={selectedMailbox ? busy===`pst-${selectedMailbox.id}` : true} type="submit">{accountPanelMode === "import" ? copy.pstImport : copy.pstExport}</button></form> : null}<div className="sublist mailbox-history">{selectedAccount.mailboxes.flatMap((mailbox)=>mailbox.pst_jobs.map((job)=><span className={job.status === "completed" ? "pill ok" : job.status === "failed" ? "pill warn" : "pill"} key={job.id}>{mailbox.display_name} · {job.direction} · {job.status} · {job.processed_messages} · {job.error_message ?? job.server_path}</span>))}</div></div> : <p className="feedback muted">{copy.selectAccountHint}</p>}</aside></div> : null}
