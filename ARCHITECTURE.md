...
- `MAPI over HTTP` as a guarded, non-advertised implementation track for future Outlook desktop support
...
- `MAPI over HTTP` work can proceed behind authenticated, non-advertised endpoints, but autodiscover must not publish it until EMSMDB, NSPI, session context, and canonical mailbox synchronization are implemented enough for real Outlook desktop login
