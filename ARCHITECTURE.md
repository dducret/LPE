...
- `MAPI over HTTP` as the primary classic Outlook for Windows Exchange-account path, with authenticated public edge routing and MAPI Autodiscover publication enabled for the current 0.5.1 baseline, after its introduction in 0.5.0; Outlook Anywhere / RPC over HTTP remains a separately gated legacy `EXPR` compatibility shim
...
The current product priority is implementing the selected `EWS` adapter
...
- `IMAP` remains a supported client communication protocol and should receive correctness fixes for sync, `UID` behavior, flags, and real-client compatibility, but it is no longer the main release driver
