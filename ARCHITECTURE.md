...
- `MAPI over HTTP` as the primary classic Outlook for Windows Exchange-account path, with authenticated public edge routing and MAPI Autodiscover publication enabled for the current 0.5.2 baseline, after its introduction in 0.5.0; Outlook Anywhere / RPC over HTTP remains a separately gated legacy `EXPR` compatibility shim
